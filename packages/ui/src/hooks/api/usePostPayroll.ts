...
interface UsePostPayrollOptions<T> {
  url: string;
  data?: any;
  salMonth?: string;
...
interface UsePostPayrollResult<T> {
  data: T | null;
  loading: boolean;
...
export const usePostPayroll = <T>({
  ...
  onError,
  onProgress
}: UsePostPayrollOptions<T>): UsePostPayrollResult<T> => {
  ...
  const post = async (postData?: any, retryCount = 0) => {
    try {
      ...
      const response = await axios({
        method: 'POST',
        url: `${API_BASE_URL}/api/payroll/engine/pipeline/${url}`,
        data: requestData,
...
const { post, loading, error, data } = usePostPayroll<any>({
  url: 'uploadfile',
  data: {
    name: 'example',
    description: 'test'
...
// For file upload
const { post, loading, error, uploadProgress } = usePostPayroll<any>({
  url: 'uploadfile',
  files: selectedFile,
  headers: {
    'X-workflow': 'workflow1',
