...
export default function ReportSelector() {
  ...
  const categories = reportData ? ["All", ...Array.from(new Set(reportData.options?.map((option: ReportOption) => option.category) || []))] : ["All"]

  // Filter reports based on active tab and search query
  const filteredReports = reportData?.options?.filter((option: ReportOption) => {
  ...
  const selectedReportLabel = reportData?.options?.find((option: ReportOption) => option.value === selectedReport)?.label || ""

  return (
    <div className="border-0 bg-gradient-to-br from-white to-gray-50 rounded-lg shadow-lg">
      <div className="p-6">
        {/* Enhanced Header */}
            ...
            <div className="relative report-dropdown">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 h-12 border-2 border-gray-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20 rounded-xl transition-all duration-200 text-base"
                />
                <div 
