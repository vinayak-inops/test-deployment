import { Calendar, Clock, FileText, Users, Building2, GitBranch, Filter, Download, RefreshCw, DownloadCloud, Eye, X, FileSpreadsheet, Search as SearchIcon, Shield, CheckCircle } from "lucide-react"
...
const FETCH_EMPLOYEE_CATEGORIES_QUERY = gql`
  query FetchEmployeeCategories($criteriaRequests: [CriteriaRequest!]!, $collection: String!) {
    fetchEmployeeCategories(criteriaRequests: $criteriaRequests, collection: $collection) {
...
export default function ReportInfoSection({ data, fileId, permission, onDownloadClick }: AttendanceReportProps) {
  ...
  // Extract shiftGroup codes and shift codes separately
  const shiftGroupCodes = useMemo(() => extractCodes(reportData?.shiftGroups || []), [reportData?.shiftGroups])
  const shiftCodes = useMemo(() => extractCodes(reportData?.shifts || []), [reportData?.shifts])
  ...
  const { data: shiftsResponse, loading: shiftsLoading } = useQuery(FETCH_SHIFTS_QUERY, {
    ...
    fetchPolicy: 'cache-and-network',
    skip: !tenantCode || (shiftGroupCodes.length === 0 && shiftCodes.length === 0),
  })
  ...
  // Fetch file when report data has a path
  useEffect(() => {
    if (reportData) {
      const filePath = reportData.report || reportData.filePath || reportData.reportPath || reportData.path || reportData.documentPath;
      // Only fetch if it's a path (starts with /) and not base64 data
      if (filePath && typeof filePath === 'string' && (filePath.startsWith('/') || filePath.startsWith('app/'))) {
        const fileType = reportData.extension?.toLowerCase() === 'pdf' 
          ? 'application/pdf' 
          : reportData.extension?.toLowerCase() === 'xlsx' || reportData.extension?.toLowerCase() === 'excel'
  ...
  // Process Excel data when fileBase64 or reportData.report changes
  useEffect(() => {
    const dataToUse = fileBase64 || reportData?.report;
    if (dataToUse && (viewerType === 'excel' || viewerType === 'xlsx' || viewerType === 'xls')) {
    ...
    } else if (fileResult?.bytes && (viewerType === 'excel' || viewerType === 'xlsx' || viewerType === 'xls')) {
      // Use bytes directly from hook
      setExcelLoading(true);
      try {
  ...
  }, [fileBase64, reportData?.report, viewerType, fileResult]);
  ...
  const handleReportDownload = async () => {
    ...
    // Use bytes from hook if available, otherwise use base64
    if (fileResult?.bytes) {
      try {
      ...
      try {
        ...
        const isPath = reportData.report.startsWith('/') || reportData.report.startsWith('app/');
        if (isPath) {
          // It's a path, we should have fetched it via hook, but if not, try to fetch now
          const fileType = getMimeType(reportData.extension || 'pdf');
          const result = await fetchByteArray(reportData.report, fileType);
          if (result.success && result.bytes) {
  ...
  const handleViewFile = async () => {
    // Use bytes from hook if available
    if (fileResult?.bytes) {
      try {
        const bytesArray = Uint8Array.from(fileResult.bytes);
        const blob = new Blob([bytesArray], {
  ...
  const normalizedEmployeeCategories = useMemo(() => {
    const categories = employeeCategoriesResponse?.fetchEmployeeCategories
    if (!categories || employeeCategoryCodes.length === 0) {
    ...
    return categories
      .filter((cat: any) => employeeCategoryCodes.includes(cat.employeeCategoryCode))
      .map((cat: any) => ({
  ...
  const normalizedShiftGroups = useMemo(() => {
    // Process shiftGroups from report data
    const reportShiftGroups = reportData?.shiftGroups || []
    
    // If no codes in report data, return empty array
    if (reportShiftGroups.length === 0) {
      return []
    }
    
    // Always prepare report data mapping first (for fallback)
    const reportDataMap = new Map<string, any>()
    reportShiftGroups.forEach((item: any) => {
      if (typeof item === 'string') {
        // String items (codes directly)
        if (!reportDataMap.has(item)) {
          reportDataMap.set(item, { code: item, name: item })
        }
      } else {
        // Object items
        const code = item.shiftGroupCode || item.code || item
        const name = item.shiftGroupName || item.name || code
        if (code && !reportDataMap.has(code)) {
          reportDataMap.set(code, { code, name })
        }
      }
    })
    
    // Always include all report data first
    // Then enhance with GraphQL data if available
    const resultMap = new Map<string, any>(reportDataMap)
    
    // If GraphQL data is available, try to enhance with names from GraphQL
    if (shiftsResponse && rawShiftGroups.length > 0) {
      const codeSet = new Set(shiftGroupCodes)
      
      // Add shift groups from GraphQL
      rawShiftGroups.forEach((sg: any) => {
        if (sg.shiftGroupCode && codeSet.has(sg.shiftGroupCode)) {
          resultMap.set(sg.shiftGroupCode, {
  ...
  }, [shiftsResponse, rawShiftGroups, shiftGroupCodes, reportData?.shiftGroups])

  const normalizedShifts = useMemo(() => {
    // Process shifts from report data (excluding shiftGroups)
    const reportShifts = reportData?.shifts || []
    ...
    reportShifts.forEach((item: any) => {
      if (typeof item === 'string') {
        ...
        // Object items
        const code = item.shiftCode || item.code || item
        const name = item.shiftName || item.name || code
        if (code && !reportDataMap.has(code)) {
          reportDataMap.set(code, { code, name })
        }
      }
    ...
    // Return all values
    return Array.from(resultMap.values())
  }, [shiftsResponse, rawShifts, shiftCodes, reportData?.shifts])
  ...
  const filteredShiftGroups = useMemo(() => filterData(normalizedShiftGroups, shiftGroupsSearch), [normalizedShiftGroups, shiftGroupsSearch])
  ...
  const paginatedShiftGroups = useMemo(() => filteredShiftGroups.slice((shiftGroupsPage - 1) * pageSize, shiftGroupsPage * pageSize), [filteredShiftGroups, shiftGroupsPage])
  ...
  useEffect(() => setShiftGroupsPage(1), [shiftGroupsSearch])
  ...
  return (
    <div className="min-h-screen py-4 md:py-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <div className="max-w-7xl mx-auto space-y-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <div className="lg:col-span-4 space-y-6">
                ...
                <div className="px-6 py-4 space-y-2">
                  ...
                  {reportData?.reportName && (
                    <div className="flex items-center border-b border-gray-100 pb-2">
                      ...
                      <span className="text-sm text-gray-900 font-medium">{reportData.reportTitle}</span>
                    </div>
                  )}
                  ...
                  {reportData?.tenantId && (
                    <div className="flex items-center border-b border-gray-100 pb-2">
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider w-32 flex-shrink-0">Tenant ID</label>
                      <span className="text-sm text-gray-900 font-medium font-mono">{reportData.tenantId}</span>
                    </div>
                  )}

                  {/* Extension */}
                  {reportData?.extension && (
                    <div className="flex items-center border-b border-gray-100 pb-2">
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider w-32 flex-shrink-0">File Type</label>
                      <span className="text-sm text-gray-900 font-medium uppercase">{reportData.extension}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
                <div className="px-6 py-4 border-b border-gray-200">
                  <div className="flex items-center justify-between">
                    <h3 className="text-base font-semibold text-gray-700">Report Period</h3>
                    {reportData?.period && (
                      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-semibold border bg-blue-50 text-blue-700 border-blue-200">
                        <Calendar className="h-4 w-4 text-blue-600" />
                        <span className="uppercase tracking-wide">{reportData.period}</span>
                      </div>
                    )}
                  </div>
                </div>
                <div className="px-6 py-4 space-y-2">
                  ...
                  {reportData?.period && (
                    <div className="flex items-center border-b border-gray-100 pb-2">
                      ...
                      <span className="text-sm text-gray-900 font-medium">{reportData.period}</span>
                    </div>
                  )}
                  ...
                  {reportData?.fromDate && reportData?.toDate && (
                    <div className="flex items-center border-b border-gray-100 pb-2">
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider w-32 flex-shrink-0">Date Range</label>
                      <span className="text-sm text-gray-900 font-medium">{dateRange.range}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              <FilterTable
                title="Shift Groups"
                icon={Clock}
                data={normalizedShiftGroups}
                filteredData={filteredShiftGroups}
                paginatedData={paginatedShiftGroups}
                searchTerm={shiftGroupsSearch}
                onSearchChange={setShiftGroupsSearch}
                page={shiftGroupsPage}
                onPageChange={setShiftGroupsPage}
                codeLabel="Shift Group Code"
                nameLabel="Shift Group Name"
              />
      ...
      {viewerOpen && viewerUrl && (
              ...
              {viewerType === 'pdf' ? (
                ...
                <div className="w-full h-full flex flex-col overflow-hidden">
                  {excelLoading || fileLoading ? (
                    ...
                    <div className="w-full h-full overflow-auto scroll-hidden bg-white p-4">
       ...
       <DownloadProgressPopup
         ...
         onDownload={loading ? handleLoadingDownload : handleReportDownload}
