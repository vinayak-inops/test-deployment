...
export default function AdminWrapper({ children }: { children: React.ReactNode }) {
    ...
    // Show loading spinner while token is loading or mode checking is in progress (only on client-side)
    if (!isMounted) {
        return <>{children}</>; // Return children during SSR to prevent hydration mismatch
    }
