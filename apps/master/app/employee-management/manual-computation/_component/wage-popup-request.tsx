...
interface WageApplicationRequest {
  ...
  stateEvent?: string;
  isDeleted?: boolean;
  // hierarchy-based info for select all
  selectAll?: boolean;
  subsidiary?: {
...
function mapBackendToWageRequest(item: any): WageApplicationRequest {
  ...
  const roleFilter = item?.roleFilter || item;
  ...
  return {
    ...
    selectAll: !!(item.selectAll ?? roleFilter.selectAll),
    subsidiary: roleFilter.subsidiary,
    division: roleFilter.division,
    department: roleFilter.department,
    subDepartment: roleFilter.subDepartment,
    section: roleFilter.section,
    locations: Array.isArray(roleFilter.locations) ? roleFilter.locations : [],
  };
}
...
export default function WageRequestsPopup({
  ...
  const { responseData: rolePermissions } = useRolePermissions({
    serviceName: "manualComputation",
    screenName: "manualComputation",
  });
  const canApprove = !!(rolePermissions as any)?.manualComputationsApprove;
  const canReject = !!(rolePermissions as any)?.manualComputationsReject;
  const canCancel = !!(rolePermissions as any)?.manualComputationsCancel;
  ...
  const selectAllCriteria = useMemo(() => {
    ...
    if (selectedRequest.subsidiary?.subsidiaryCode) {
      criteria.push({
        field: "deployment.subsidiary.subsidiaryCode",
        operator: "eq",
        value: selectedRequest.subsidiary.subsidiaryCode,
      });
    }
    if (selectedRequest.division?.divisionCode) {
      criteria.push({
        field: "deployment.division.divisionCode",
        operator: "eq",
        value: selectedRequest.division.divisionCode,
      });
    }
    if (selectedRequest.department?.departmentCode) {
      criteria.push({
        field: "deployment.department.departmentCode",
        operator: "eq",
        value: selectedRequest.department.departmentCode,
      });
    }
    if (selectedRequest.subDepartment?.subDepartmentCode) {
      criteria.push({
        field: "deployment.subDepartment.subDepartmentCode",
        operator: "eq",
        value: selectedRequest.subDepartment.subDepartmentCode,
      });
    }
    if (selectedRequest.section?.sectionCode) {
      criteria.push({
        field: "deployment.section.sectionCode",
        operator: "eq",
        value: selectedRequest.section.sectionCode,
      });
    }
    if (selectedRequest.locations && selectedRequest.locations.length > 0) {
      criteria.push({
        field: "deployment.location.locationCode",
        operator: "in",
        value: selectedRequest.locations,
      });
    }
  ...
  // Role criteria filters (locations)
  const filteredLocations = useMemo(() => {
    if (!selectedRequest?.locations) return [];
    const q = locationSearch.trim().toLowerCase();
    if (!q) return selectedRequest.locations;
    return selectedRequest.locations.filter((code) => code.toLowerCase().includes(q));
  }, [selectedRequest?.locations, locationSearch]);

  const paginatedLocations = useMemo(() => {
    return filteredLocations.slice((locationPage - 1) * rolePageSize, locationPage * rolePageSize);
                  ...
                  <div className="max-w-2xl">
                    ...
                    {selectedRequest.employeeIDList?.length ? (
                      ...
                      <div className="mt-6 space-y-3">
                        ...
                        {(selectedRequest.subsidiary ||
                          selectedRequest.division ||
                          selectedRequest.department ||
                          selectedRequest.subDepartment ||
                          ...
                          <div className="rounded-lg border border-gray-200 bg-white px-3 py-2 shadow-sm space-y-3">
                            <div className="text-sm text-gray-900 font-semibold">Hierarchy criteria</div>

                            {/* Subsidiary */}
                            <div className="rounded-md border border-gray-200 bg-white">
                              <div className="px-3 py-2 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                                <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Subsidiary</span>
                              </div>
                              <Table>
                                <TableHeader>
                                ...
                                <TableBody>
                                  <TableRow>
                                    <TableCell className="py-1 pl-3 text-xs text-gray-800">1</TableCell>
                                    <TableCell className="py-1 text-xs text-gray-800">
                                      {selectedRequest.subsidiary?.subsidiaryCode || "-"}
                                    </TableCell>
                                    <TableCell className="py-1 pr-3 text-xs text-gray-800">
                                      {selectedRequest.subsidiary?.subsidiaryName ||
                                        selectedRequest.subsidiary?.subsidiaryCode ||
                                        "-"}
                                    </TableCell>
                                  </TableRow>
                            ...
                            <div className="rounded-md border border-gray-200 bg-white">
                              <div className="px-3 py-2 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                                <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Division</span>
                              </div>
                              <Table>
                                <TableHeader>
                                  <TableRow className="bg-gray-50">
                                ...
                                <TableBody>
                                  <TableRow>
                                    <TableCell className="py-1 pl-3 text-xs text-gray-800">1</TableCell>
                                    <TableCell className="py-1 text-xs text-gray-800">
                                      {selectedRequest.division?.divisionCode || "-"}
                                    </TableCell>
                                    <TableCell className="py-1 pr-3 text-xs text-gray-800">
                                      {selectedRequest.division?.divisionName ||
                                        selectedRequest.division?.divisionCode ||
                                        "-"}
                                    </TableCell>
                                  </TableRow>
                            ...
                            <div className="rounded-md border border-gray-200 bg-white">
                              <div className="px-3 py-2 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                                <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Department</span>
                              </div>
                              <Table>
                                <TableHeader>
                                  <TableRow className="bg-gray-50">
                                ...
                                <TableBody>
                                  <TableRow>
                                    <TableCell className="py-1 pl-3 text-xs text-gray-800">1</TableCell>
                                    <TableCell className="py-1 text-xs text-gray-800">
                                      {selectedRequest.department?.departmentCode || "-"}
                                    </TableCell>
                                    <TableCell className="py-1 pr-3 text-xs text-gray-800">
                            ...
                            <div className="rounded-md border border-gray-200 bg-white">
                              <div className="px-3 py-2 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                                <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Sub Department</span>
                              </div>
                              <Table>
                                <TableHeader>
                                  <TableRow className="bg-gray-50">
                                ...
                                <TableBody>
                                  <TableRow>
                                    <TableCell className="py-1 pl-3 text-xs text-gray-800">1</TableCell>
                                    <TableCell className="py-1 text-xs text-gray-800">
                                      {selectedRequest.subDepartment?.subDepartmentCode || "-"}
                                    </TableCell>
                                    <TableCell className="py-1 pr-3 text-xs text-gray-800">
                                      {selectedRequest.subDepartment?.subDepartmentName ||
                                        selectedRequest.subDepartment?.subDepartmentCode ||
                                        "-"}
                                    </TableCell>
                                  </TableRow>
                              ...
                              <Table>
                                ...
                                <TableBody>
                                  <TableRow>
                                    <TableCell className="py-1 pl-3 text-xs text-gray-800">1</TableCell>
                                    <TableCell className="py-1 text-xs text-gray-800">
                                      {selectedRequest.section?.sectionCode || "-"}
                                    </TableCell>
                                    <TableCell className="py-1 pr-3 text-xs text-gray-800">
                                      {selectedRequest.section?.sectionName ||
                                        selectedRequest.section?.sectionCode ||
                                        "-"}
                                    </TableCell>
                                  </TableRow>
                            ...
                            <div className="rounded-md border border-gray-200 bg-white">
                              ...
                              <Table>
                                ...
                                <TableBody>
                                  {paginatedLocations.length ? (
                                    paginatedLocations.map((code, idx) => (
                                      <TableRow key={code}>
                                        <TableCell className="py-1 pl-3 text-xs text-gray-800">
                                          {(locationPage - 1) * rolePageSize + idx + 1}
                                        </TableCell>
                              ...
                              {filteredLocations.length > rolePageSize && (
                                <div className="flex items-center justify-between px-3 py-2 border-t bg-slate-50">
                                  <p className="text-[11px] text-gray-500">
                                    Showing{" "}
                                    <span className="font-semibold">
                                      {Math.min((locationPage - 1) * rolePageSize + 1, filteredLocations.length)}-
                                      {Math.min(locationPage * rolePageSize, filteredLocations.length)}
                                    </span>{" "}
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <Button
                                      ...
                                      size="sm"
                                      className="h-6 px-2 text-[11px]"
                                      disabled={locationPage * rolePageSize >= filteredLocations.length}
                                      onClick={() =>
                                        setLocationPage((p) =>
                                          p * rolePageSize >= filteredLocations.length ? p : p + 1
                                        )
                                      }
                                    >
                                      Next
