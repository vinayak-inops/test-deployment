...
interface AddContractEmployeePopupProps {
  isOpen: boolean
  onClose: () => void
  onSelect: (employee: ContractEmployee) => void
}
...
export default function AddContractEmployeePopup({ isOpen, onClose, onSelect }: AddContractEmployeePopupProps) {
  ...
  const [searchResults, setSearchResults] = useState<ContractEmployee[]>([])
  ...
  const { refetch: refetchCount } = useRequest<any>({
    ...
    dependencies: [],
    onSuccess: (data: any) => setTotalCount(typeof data === "number" ? data : 0),
    onError: (error: any) => console.error("Error fetching contract employee count:", error),
  })

  const { data: employeesData, loading: requestLoading, error: queryError } = useQuery(FETCH_EMPLOYEES_QUERY, {
    client,
  ...
  useEffect(() => {
    if (employeesData?.fetchEmployees) {
      const mapped: ContractEmployee[] = (employeesData.fetchEmployees || []).map((e: any) => ({
        _id: e._id,
  ...
  const { post: postApprover, loading: postLoading } = usePostRequest<any>({
    ...
    onSuccess: (data: any) => {
      ...
      if (createdId) {
        router.push(`/employee-management/company-employee-approver?mode=edit&id=${createdId}`)
      } else {
        router.push(`/employee-management/company-employee-approver`)
      }
    },
    onError: (error) => {
  ...
  const handleCreateApprover = async (employee: ContractEmployee) => {
    ...
    try {
      ...
      if (existing.length > 0) {
        setIsProcessing(false)
        setErrorMessage(`EmployeeID "${selectedEmployeeId}" already exists. Cannot create duplicate approver.`)
        return
      }
      ...
      setIsProcessing(false)
      setErrorMessage(err?.response?.data?.message || err?.message || "Failed to validate or create approver. Please try again.")
        ...
        <div className="px-5 py-3 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-500 mt-0.5">Search employee and create a new employee approver record.</p>
            </div>
          </div>
                ...
                <Table className="min-w-[1400px]">
                  <TableHeader>
                    <TableRow className="bg-slate-50 hover:bg-slate-50">
                      ...
                      <TableHead className="py-2 text-[11px] font-semibold text-slate-600 uppercase tracking-wide whitespace-nowrap min-w-[150px]">Subsidiary</TableHead>
                      <TableHead className="py-2 text-[11px] font-semibold text-slate-600 uppercase tracking-wide whitespace-nowrap min-w-[150px]">Division</TableHead>
                      <TableHead className="py-2 text-[11px] font-semibold text-slate-600 uppercase tracking-wide whitespace-nowrap min-w-[150px]">Department</TableHead>
                      <TableHead className="py-2 text-[11px] font-semibold text-slate-600 uppercase tracking-wide whitespace-nowrap min-w-[150px]">Location</TableHead>
                  ...
                  <TableBody>
                    {searchResults.map((employee, index) => {
                      ...
                      const name = [employee.firstName, employee.middleName, employee.lastName].filter(Boolean).join(" ") || employee.employeeID || "-"
                      return (
                        <TableRow key={employee._id} className="hover:bg-slate-50/80 odd:bg-white even:bg-slate-50/60 transition-colors">
                          ...
                          <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{employee.deployment?.subsidiary?.subsidiaryName || "-"}</TableCell>
                          <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{employee.deployment?.division?.divisionName || "-"}</TableCell>
                          <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{employee.deployment?.department?.departmentName || "-"}</TableCell>
                          <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{employee.deployment?.location?.locationName || "-"}</TableCell>
