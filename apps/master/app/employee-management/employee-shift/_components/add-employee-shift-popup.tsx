...
interface AddEmployeeShiftPopupProps {
    isOpen: boolean;
    onClose: () => void;
    basePath: string;
    onSuccess?: () => void;
...
export default function AddEmployeeShiftPopup({
    isOpen,
    onClose,
    basePath,
    onSuccess,
}: AddEmployeeShiftPopupProps) {
    ...
    const formatName = (
        emp: { firstName?: unknown; middleName?: unknown; lastName?: unknown; employeeID?: unknown }
    ) => {
    ...
    // Build GraphQL query dynamically with hierarchyFilters and userEntitlement inline (matching employee-search.tsx)
    ...
    // Build query variables (matching employee-search.tsx pattern)
    ...
    // Build count request data (matching employee-search.tsx pattern)
    ...
    // POST request for creating employee shift record
    const { post: postEmployeeShift, loading: postLoading } = usePostRequest<any>({
        url: "employee_shift",
        onSuccess: (data: any) => {
            const createdId =
                Array.isArray(data) && data.length > 0 ? data[0]._id : data?._id;
            onClose();
            onSuccess?.();
            if (createdId) {
                router.push(`${basePath}?mode=edit&id=${encodeURIComponent(createdId)}`);
            } else {
        ...
        onError: (error) => {
            ...
            setErrorMessage("Failed to create employee shift record. Please try again.");
            console.error("Error creating employee shift:", error);
        },
    });

    const handleCreateEmployeeShift = async (employee: Employee) => {
        ...
        try {
            ...
            await postEmployeeShift(payload);
        } catch (err: any) {
            setIsProcessing(false);
            setErrorMessage(
                err?.response?.data?.message ||
                    err?.message ||
                    "Failed to create employee shift record. Please try again."
            );
            console.error("Error in handleCreateEmployeeShift:", err);
        }
    };
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p className="text-[11px] text-gray-500 mt-0.5">
                                Search contract employee and create a new employee shift record.
                            </p>
                        </div>
                    </div>
                    <button
                                                    ...
                                                    >
                                                        <div className="flex justify-end">
                                                            <Button
                                                                ...
                                                                onClick={() =>
                                                                    handleCreateEmployeeShift(employee)
                                                                }
                                                                disabled={postLoading || isProcessing}
                                                                title="Create new employee shift record"
                                                            >
                                                                {isProcessing ? (
