...
import { Edit, MoreVertical, Trash2, Plus, Loader2, Calendar, ChevronLeft, ChevronRight } from "lucide-react"
...
interface EmployeeTrainingCompletion {
  _id: string
  employeeID: string
...
interface EmployeeTrainingCompletionTableProps {
  data: EmployeeTrainingCompletion[]
  onView: (row: EmployeeTrainingCompletion) => void
  onEdit?: (row: EmployeeTrainingCompletion) => void
  onDelete?: (row: EmployeeTrainingCompletion) => void
  onAdd?: () => void
  permission?: {
    view?: boolean
    edit?: boolean
...
const formatTrainingList = (trainings: string[] | undefined, maxVisible: number = 3): string => {
  if (!trainings || trainings.length === 0) return "-"
  if (trainings.length <= maxVisible) return trainings.join(", ")
  const visible = trainings.slice(0, maxVisible).join(", ")
  const remaining = trainings.length - maxVisible
...
export default function EmployeeTrainingCompletionTable({
  data,
  onView,
  onEdit,
  onDelete,
  ...
  isLoading = false,
  externalPagination,
}: EmployeeTrainingCompletionTableProps) {
      ...
      <CardHeader className="border-b px-5 py-2 bg-gray-50/50">
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm text-muted-foreground">
            {isLoading ? "Loading..." : `${totalItems} records`}
          </span>
        </div>
      </CardHeader>
            ...
            <Table className="min-w-[1000px]">
              <TableHeader>
                <TableRow className="bg-blue-50">
                  ...
                  <TableHead className="px-5 py-2 min-w-[150px] whitespace-nowrap">Employee ID</TableHead>
                  <TableHead className="px-5 py-2 min-w-[300px] whitespace-nowrap">Trainings Completed</TableHead>
                ...
                {data.map((row, index) => (
                  ...
                  >
                    ...
                    <TableCell className="px-5 py-2 text-sm">
                      {formatTrainingList(row.trainingsCompleted)}
                    </TableCell>
