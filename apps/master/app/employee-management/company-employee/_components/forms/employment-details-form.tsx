...
export function EmploymentDetailsForm({ 
  ...
  const contractorQueryVariables = useMemo(
    ...
    [debouncedTenantCode, userEntitlement.contractor]
  )

  const { loading: contractorLoading } = useQuery(FETCH_CONTRACTORS_QUERY, {
    client,
    variables: contractorQueryVariables,
    skip: !debouncedTenantCode,
    fetchPolicy: "network-only",
    onCompleted: (data: any) => {
      const contractors = Array.isArray(data?.fetchContractors) ? data.fetchContractors : []
      setContractorData(contractors.filter((rec: any) => rec?.isDeleted !== true))
    },
    onError: (error: any) => {
  ...
  const { refetch: fetchCompanyEmp } = useRequest<any[]>({
    url: `map/company_employee/search?tenantCode=${tenantCode}`,
    onSuccess: (data: any) => {
      const activeCompanyEmp = (data || []).filter((rec: any) => rec?.isDeleted !== true);
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="space-y-2 group">
                <Label htmlFor="superviser" className="block text-xs font-medium text-gray-700 uppercase tracking-wide">
                  {getFieldLabel("superviser", "Supervisor")} {isFieldRequired("superviser") && <span className="text-red-500">*</span>}
                </Label>
                <Input
                  id="superviser"
                  {...register("superviser")}
                  disabled={isViewMode}
                  className={fieldClassName(Boolean(showErrors && errors.superviser), isViewMode)}
                  placeholder="Enter supervisor ID"
                />
                {showErrors && errors.superviser && (
                  <p className="text-red-500 text-sm mt-1 flex items-center gap-1">
                    <X className="h-3 w-3" />
                    {errors.superviser.message}
                  </p>
                )}
              </div>
              )}
