...
export default function ContractEmployeeApproverApplication() {
  ...
  const queryVariables = useMemo(() => {
    ...
    return {
      criteriaRequests,
      applicationMode: "clms", // Required field
      offset,
      limit: itemsPerPage,
    }
  ...
  const { loading: requestLoading, refetch, data, error } = useQuery(GET_APPROVER_DETAILS, {
    ...
    onCompleted: (data) => {
      ...
      console.log("Processed Rows Array:", rows)
  ...
  const handleView = useCallback((row: ContractEmployeeApproverItem) => {
    if (!viewMode) return
    if (!row?._id) return
    router.push(`/employee-management/contract-employee-approver?mode=view&id=${row._id}`)
  }, [router, viewMode])

  const handleEdit = useCallback((row: ContractEmployeeApproverItem) => {
    if (!editMode) return
    if (!row?._id) return
    router.push(`/employee-management/contract-employee-approver?mode=edit&id=${row._id}`)
  }, [router, editMode])
