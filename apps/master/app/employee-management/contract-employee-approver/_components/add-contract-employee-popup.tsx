...
export default function AddContractEmployeePopup({ isOpen, onClose, onSelect }: AddContractEmployeePopupProps) {
  ...
  const { data: employeesResponse, loading: requestLoading, error: queryError } = useQuery(FETCH_COMPANY_EMPLOYEE_QUERY, {
    client,
  ...
  const employeesData = useMemo<ContractEmployee[]>(() => {
  ...
  const { post: postApprover, loading: postLoading } = usePostRequest<any>({
    ...
    onSuccess: (data: any) => {
      ...
      if (createdId) {
        router.push(`/employee-management/contract-employee-approver?mode=edit&id=${createdId}`)
      } else {
        ...
        <div className="px-5 py-3 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-500 mt-0.5">Search employee and create a new contract employee approver record.</p>
            </div>
          </div>
