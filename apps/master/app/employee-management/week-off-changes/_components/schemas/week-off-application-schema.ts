...
export const weekOffSchema = z.object({
  ...
  _id: z
    ...
    .refine((val) => !val || !hasDisallowedChars(val), {
      message: "_id can only contain letters, numbers, spaces, underscores (_), and hyphens (-)"
    }),
  parentID: z
    .string()
    .optional()
    .refine((val) => !val || !hasDisallowedChars(val), {
      message: "Parent ID can only contain letters, numbers, spaces, underscores (_), and hyphens (-)"
    }),
  parentId: z
    .string()
    .optional()
