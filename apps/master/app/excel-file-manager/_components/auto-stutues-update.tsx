...
const TimelineItem = ({ item, isLast, extension, reportData, onReportData, permission, onDownloadClick }: any) => {
  ...
  const { fetchByteArray } = useByteToBase64({
    onError: (error) => {
      console.error('Error fetching file in TimelineItem:', error);
    }
  });
  ...
  const handleTimelineReportDownload = async () => {


    if (item.report && typeof item.report === 'string') {
      try {
        // Check if it's a path or base64
        const isPath = item.report.startsWith('/') || item.report.startsWith('app/');
        if (isPath) {
          // It's a path, fetch it via hook
          const fileType = getMimeType(extension);
          const result = await fetchByteArray(item.report, fileType);
          if (result.success && result.bytes) {
            ...
            link.href = url;
            const safeName = getSafeFileName(`${tenantCode}_${reportData?.reportName}` || 'report');
            const fileExt = getFileExtension(extension);
            link.download = `${safeName}.${fileExt}`;
        ...
        const link = document.createElement('a');
        link.href = url;
        // Use reportTitle from reportData if available, sanitize, and map extension
        const safeName = getSafeFileName(`${tenantCode}_${reportData?.reportName}` || 'report');
        const fileExt = getFileExtension(extension);
        link.download = `${safeName}.${fileExt}`;
        document.body.appendChild(link);
  ...
  const handleReportDownload = async () => {
    if (reportData?.report) {
      try {
        // Check if it's a path or base64
        const isPath = typeof reportData.report === 'string' &&
          (reportData.report.startsWith('/') || reportData.report.startsWith('app/'));
        if (isPath) {
          // It's a path, fetch it via hook
          const fileType = getMimeType(extension);
  ...
  // Map generic/alias extensions to proper file extensions for download
  ...
  return (
    <div className="mb-6 relative pl-8">
      ...
      >
        <div className="flex items-center text-sm border-b-2 border-gray-100 pb-1.5 mb-1.5">
          <div className="flex items-center">
            <UserCircle size={14} className="text-blue-600 mr-2" />
            <span className="font-semibold text-gray-700">{item.stateName}</span>
          </div>
        ...
        <Card className="shadow-sm mt-3 p-0">
          ...
          <CardFooter className="flex justify-between items-center border-t border-gray-100 px-3 py-3 mt-3">
            ...
            <button
              ...
              onClick={onDownloadClick || (hasReport ? handleTimelineReportDownload : handleReportDownload)}
            >
...
export default function AutoStatusUpdate({
  ...
  const tenantCode = useGetTenantCode();
  const [fallbackTimelineData, setFallbackTimelineData] = useState<any[]>([]);
  const [sseTimelineData, setSseTimelineData] = useState<any[]>([]);
  ...
  const {
    ...
    onSuccess: (data: any) => {
      if (data) {
        ...
        setFallbackTimelineData(timelineArray);
  ...
  useEffect(() => {
    ...
    const sseUrl = `${baseUrl}/api/query/attendance/sse?tenant=${encodeURIComponent(tenantCode)}&fileId=${encodeURIComponent(String(fileId))}`;
    ...
    eventSource.onmessage = (event) => {
      ...
      try {
        ...
        const nextItem = parsed?.data || parsed;
        const nextFileId = nextItem?.fileId || nextItem?.workflowType || parsed?.id;

        if (String(nextFileId) !== String(fileId)) return;

        setSseTimelineData((prev) => {
          const nextId = nextItem?._id || parsed?.id || `${nextItem?.timestamp}-${nextItem?.eventName}`;
          const exists = prev.some((item) => {
            const itemId = item?._id || `${item?.timestamp}-${item?.eventName}`;
          ...
          const merged = exists
            ? prev.map((item) => {
                const itemId = item?._id || `${item?.timestamp}-${item?.eventName}`;
                return itemId === nextId ? nextItem : item;
              })
            : [nextItem, ...prev];
        ...
        console.error("SSE message parse failed:", error, event.data);
      }
    };
  ...
  // Show download popup during loading
  useEffect(() => {
    if (loading && fileId) {
      // Trigger download popup in parent component
    }
  }, [loading, fileId]);

  // Ensure timelineData is always an array
  const safeSseTimelineData = Array.isArray(sseTimelineData) ? sseTimelineData : [];
  const safeFallbackTimelineData = Array.isArray(fallbackTimelineData) ? fallbackTimelineData : [];
  const isUsingSseTimeline = sseStatus === "connected" && safeSseTimelineData.length > 0;
  const safeTimelineData = isUsingSseTimeline ? safeSseTimelineData : safeFallbackTimelineData;

  return (
    <div className="border-0 overflow-y-auto h-full scroll-smooth [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-gray-400">
      <div className="p-4 relative">
        ...
        <div className="flex justify-between items-start mb-4 pb-4 border-b border-gray-200">
          <div>
            <h3 className="text-base font-semibold text-gray-700 mb-1">Status Updates</h3>
            <p className="text-xs text-gray-500">
              {isUsingSseTimeline
                ? "Live SSE updates connected"
                : sseStatus === "connecting"
                  ? "Connecting to live updates..."
                  : "Live updates unavailable. Showing saved status."}
            </p>
          </div>
        </div>

        {/* Loading and Error States */}
        {!isUsingSseTimeline && loading && (
          <div className="text-center py-8">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto"></div>
            <p className="text-gray-500 mt-2">Loading status updates...</p>
          </div>
        )}
        {!isUsingSseTimeline && error && (
        ...
        {!loading && !error && safeTimelineData.length === 0 && (
          <div className="flex flex-col items-center justify-center text-center p-8 mt-8">
            <div className="flex items-center justify-center h-12 w-12 bg-amber-100 rounded-full pulse-animation">
              <Clock className="h-6 w-6 text-amber-600" />
            </div>
            <h3 className="mt-4 text-base font-semibold text-gray-700">Status is Updating</h3>
            <p className="mt-2 text-xs text-gray-500 max-w-md px-4">
              We are processing your file. Status updates will appear here shortly.
            </p>
          </div>
        )}
        {(isUsingSseTimeline || (!loading && !error)) && safeTimelineData.map((item, idx) => (
          <TimelineItem
            key={item._id || idx}
            item={item}
