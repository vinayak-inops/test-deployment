...
export function BasicInformationDraftForm({
  isOpen = true,
  onClose,
  title = "Contractor - Basic Information",
  ...
  const {
    ...
    onSuccess: (response) => {
      ...
      router.push("/contractor-management/contractor")
    },
    onError: (error) => {
    ...
    onSuccess: (data) => {
      if (Array.isArray(data) && data[0] && data[0].isDeleted !== true) {
        ...
        reset({
          ...
          contractorImage: row.contractorImage || "",
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      data: {
        ...
        companyDetailstab: false,
        licensetab: false,
        bankDetailtab: false,
        securityDeposittab: false,
        addressInformationtab: false,
        documenttab: false,
        workOrdertab: false,
        penaltytab: false,
        wcPolicytab: false,
    ...
    await postBasicInformation(payload)
  }
      ...
      <div className="bg-transparent w-full max-w-6xl flex flex-col">
        ...
        <div className="flex items-center justify-between py-2 border-t border-gray-100 pt-4">
          <div className="space-y-0.5">
            ...
            <p className="text-xs text-gray-500">Enable or disable this contractor</p>
          </div>
          <Switch
            disabled={isViewMode}
