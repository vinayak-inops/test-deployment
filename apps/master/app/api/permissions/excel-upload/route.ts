...
export async function POST(request: NextRequest) {
  try {
    ...
    let excelUploadEntryFound = false;

    const updatedScreenPermissions = existingScreenPermissions.map((sp) => {
      if (sp.serviceName !== "excel-upload") return sp;

      excelUploadEntryFound = true;
      const existingScreens = sp.screens || [];

      let excelFileManagerScreen = existingScreens.find((s: any) => s.screenName === "excelFileManager");
      
      if (!excelFileManagerScreen) {
        excelFileManagerScreen = {
          screenName: "excelFileManager",
          permissions: {},
        };
      }

      const updatedExcelFileManager = {
        ...excelFileManagerScreen,
        permissions: {
          ...(excelFileManagerScreen.permissions || {}),
          excelUpload: !!data.excelUpload,
        },
      };

      const excelFileManagerPerms = updatedExcelFileManager.permissions || {};
      const excelFileManagerIsActive = Object.values(excelFileManagerPerms).some((v) => !!v);

      const mergedScreens = existingScreens
        .filter((s: any) => s.screenName !== "excelFileManager")
        .map((screen: any) => {
      ...
      mergedScreens.push({
        ...updatedExcelFileManager,
        isActive: excelFileManagerIsActive,
      });
    ...
    // If excel-upload entry doesn't exist, create it
    if (!excelUploadEntryFound) {
      const newExcelFileManager = {
        screenName: "excelFileManager",
        permissions: {
          excelUpload: !!data.excelUpload,
        },
      };
      const excelFileManagerPerms = newExcelFileManager.permissions || {};
      const excelFileManagerIsActive = Object.values(excelFileManagerPerms).some((v) => !!v);

      const isActive = excelFileManagerIsActive;
    ...
    console.error("Error in /api/permissions/excel-upload:", error);
