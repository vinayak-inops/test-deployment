...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      entitlementCode,
      tenantCode,
      data, // { screens: [{ screenName, permissions }] }
    } = body;
    ...
    const updatedScreenPermissions = existingScreenPermissions.map((sp) => {
      if (sp.serviceName !== "role") return sp;
    ...
    // If role entry doesn't exist, create it
    if (!roleEntryFound && updatedScreensFromForm.length > 0) {
    ...
    console.error("Error in /api/permissions/role:", error);
