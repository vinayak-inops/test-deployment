...
export async function POST(request: NextRequest) {
  try {
    ...
    let dashboardEntryFound = false;

    const updatedScreenPermissions = existingScreenPermissions.map((sp) => {
      if (sp.serviceName !== "dashboard") return sp;

      dashboardEntryFound = true;
      const existingScreens = sp.screens || [];

      let liveDashboardScreen = existingScreens.find((s: any) => s.screenName === "liveDashboard");
      
      if (!liveDashboardScreen) {
        liveDashboardScreen = {
      ...
      const updatedLiveDashboard = {
        ...liveDashboardScreen,
        permissions: {
          ...(liveDashboardScreen.permissions || {}),
      ...
      const liveDashboardPerms = updatedLiveDashboard.permissions || {};
      const liveDashboardIsActive = Object.values(liveDashboardPerms).some((v) => !!v);

      const mergedScreens = existingScreens
        .filter((s: any) => s.screenName !== "liveDashboard")
        .map((screen: any) => {
      ...
      mergedScreens.push({
        ...updatedLiveDashboard,
        isActive: liveDashboardIsActive,
      });

      // service-level isActive true if any permission flag in any dashboard screen is truthy
    ...
    // If dashboard entry doesn't exist, create it
    if (!dashboardEntryFound) {
      const newLiveDashboard = {
        screenName: "liveDashboard",
        permissions: {
          totalCount: !!data.totalCount,
          workOrderCount: !!data.workOrderCount,
      ...
      const liveDashboardPerms = newLiveDashboard.permissions || {};
      const liveDashboardIsActive = Object.values(liveDashboardPerms).some((v) => !!v);

      const isActive = liveDashboardIsActive;
    ...
    console.error("Error in /api/permissions/dashboard:", error);
