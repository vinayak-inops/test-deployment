...
interface CompanyEmployeeDraftRow {
  _id: string;
  employeeID?: string;
  firstName?: string;
  middleName?: string;
...
interface AddCompanyEmployeeDraftInfoPopupProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export default function AddCompanyEmployeeDraftInfoPopup({
  isOpen,
  onClose,
}: AddCompanyEmployeeDraftInfoPopupProps) {
  ...
  const [rows, setRows] = useState<CompanyEmployeeDraftRow[]>([]);
  ...
  const { refetch: refetchCount } = useRequest<any>({
    url: "draft/company_employee/count",
    method: "POST",
    ...
    onError: (error) => {
      console.error("Error fetching draft company employee count:", error);
      setTotalCount(0);
    },
  ...
  const { loading, refetch } = useRequest<any>({
    url: `draft/company_employee/search?offset=${offset}&limit=${limit}`,
    method: "POST",
    data: criteriaRequests,
    onSuccess: (data: any) => {
      ...
      setRows(
        list.map((c: any) => ({
          ...
          lastName: c?.lastName || "",
          gender: c?.gender || "",
          joiningDate: c?.joiningDate || c?.dateOfJoining || "",
          emailID: c?.emailID || "",
    ...
    onError: (error) => {
      console.error("Error fetching draft company employees:", error);
      setRows([]);
    },
    ...
    >
      <div className="bg-white rounded-lg shadow-xl border border-gray-200 w-full max-w-6xl max-h-[92vh] overflow-hidden flex flex-col">
        <div className="px-5 py-3 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-gray-900">Draft Company Employee Information</h3>
            <p className="text-[11px] text-gray-500 mt-0.5">Showing basic details from draft company employee records.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            ...
            <div className="flex w-[240px] items-center border-r px-2 py-1.5">
              ...
              <Select value={searchField} onValueChange={setSearchField}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="emailID">Email</SelectItem>
            ...
            <Table className="min-w-[1600px]">
              <TableHeader>
                <TableRow className="bg-slate-50 hover:bg-slate-50">
                  ...
                  <TableHead className="py-2 text-[11px] font-semibold text-slate-600 uppercase tracking-wide whitespace-nowrap">First Name</TableHead>
                  <TableHead className="py-2 text-[11px] font-semibold text-slate-600 uppercase tracking-wide whitespace-nowrap">Middle Name</TableHead>
                  <TableHead className="py-2 text-[11px] font-semibold text-slate-600 uppercase tracking-wide whitespace-nowrap">Last Name</TableHead>
                  ...
                  <TableHead className="py-2 text-[11px] font-semibold text-slate-600 uppercase tracking-wide whitespace-nowrap">Joining Date</TableHead>
              ...
              <TableBody>
                {loading ? (
                  <TableRow>
                    <TableCell className="py-8 text-center text-gray-500" colSpan={11}>Loading employees...</TableCell>
                  </TableRow>
                ) : rows.length === 0 ? (
                  <TableRow>
                    <TableCell className="py-8 text-center text-gray-500" colSpan={11}>No draft company employees found.</TableCell>
                  </TableRow>
                ) : (
                  rows.map((employee, idx) => (
                    <TableRow key={`${employee._id}-${idx}`} className="hover:bg-slate-50/80 odd:bg-white even:bg-slate-50/60 transition-colors">
                      ...
                      <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{safeText(employee.firstName)}</TableCell>
                      <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{safeText(employee.middleName)}</TableCell>
                      <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{safeText(employee.lastName)}</TableCell>
                      <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{safeText(employee.gender)}</TableCell>
                      <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{safeText(employee.joiningDate)}</TableCell>
                      <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{safeText(employee.emailID)}</TableCell>
                      <TableCell className="py-1.5 font-mono text-[11px] text-gray-700 whitespace-nowrap">{safeText(employee.aadharNumber)}</TableCell>
                      <TableCell className="py-1.5 text-xs text-gray-700 whitespace-nowrap">{safeText(employee.status)}</TableCell>
                      <TableCell className="py-1.5 pr-4 text-right sticky right-0 bg-white z-10">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            onClose();
                            router.push(`/company_management/company-employee?form=temp&mode=edit&id=${encodeURIComponent(encryptedData)}`);
                          }}
                        >
