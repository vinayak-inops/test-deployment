...
export default function MailGroupAssociationPage() {
  ...
  const { responseData: rolePermissions } = useRolePermissions({
    serviceName: "setting",
    screenName: "mailGroupAssociationHris",
  })
  ...
  const { post: createAssociationFromCode } = usePostRequest<any>({
    ...
    onSuccess: (response) => {
      ...
      if (createdId) {
        ...
        router.push(`/settings/mail-group-association-hris?mode=edit&id=${encodeURIComponent(String(createdId))}`)
        return
      }
      if (pendingMailGroupCode) {
        setIsMailGroupPopupOpen(false)
        router.push(`/settings/mail-group-association-hris?mode=add&mailGroupCode=${encodeURIComponent(pendingMailGroupCode)}`)
      }
    },
  })
    ...
    return (
      <>
        <MailGroupAssociationForm
          ...
          layout="page"
          onBack={() => {
            router.push("/settings/mail-group-association-hris")
          }}
          recordId={rowId}
      ...
      <div>
        ...
        <MailGroupAssociationTable
          ...
          onEdit={(raw) => {
            ...
            if (!id) return
            router.push(`/settings/mail-group-association-hris?mode=edit&id=${encodeURIComponent(String(id))}`)
          }}
          onView={(raw) => {
            const id = raw?._id?.$oid || raw?._id
            if (!id) return
            router.push(`/settings/mail-group-association-hris?mode=view&id=${encodeURIComponent(String(id))}`)
          }}
        />
      </div>
