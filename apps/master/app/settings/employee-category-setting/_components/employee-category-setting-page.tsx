...
import {
  useEmployeeCategorySettingLogic,
  FIELD_LABELS,
  type EmpCategorySettingSearchField,
} from "./hooks/useEmployeeCategorySettingLogic"
import EmpCategorySettingTable from "./employee-category-setting-table"
import EmpCategorySettingAddPopup from "./employee-category-setting-add-popup" // Import the popup component
import EmpCategorySettingFormController from "./employee-category-setting-form-controller" // Import the form controller

export default function EmpCategorySettingPage() {
  ...
  if (isFormMode && currentMode) {
    return (
      <EmpCategorySettingFormController
        mode={currentMode}
      ...
      <TableHeader
        ...
        description={
          loading || countLoading
            ? "Employee Category Setting | Loading..."
            : `Employee Category Setting | ${totalCount} records`
        }
        canAdd={addMode}
        onAddNew={handleAdd}
        ...
        <div className="max-w-7xl mx-auto flex items-center gap-4">
          <div className="flex bg-muted/50 rounded-lg border flex-1">
            <div className="flex items-center bg-background border-r rounded-l-lg px-3 py-2 w-52">
              <Select
                value={searchField}
                onValueChange={(v) => setSearchField(v as EmpCategorySettingSearchField)}
              >
                <SelectTrigger className="w-full h-6 border-none p-0 text-sm font-medium text-foreground focus:ring-0 bg-transparent shadow-none">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="employeeCategoryCode">Category Code</SelectItem>
                  <SelectItem value="sourceEmailAddess">Source Email</SelectItem>
                </SelectContent>
              </Select>
            </div>
