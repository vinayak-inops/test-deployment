...
export default function SchedulerNotificationTab({ form, viewMode, mode: _mode, showErrors = false }: Props) {
  ...
  const { data } = useQuery(FETCH_EMPLOYEES_QUERY_BASE, {
    client,
    variables: {
  ...
  const employeeEmailOptions = useMemo(() => {
    const employees = (data?.fetchEmployees as Array<{
      isDeleted?: boolean;
    ...
    employees
      ...
      .filter((rec) => {
        const email = rec?.emailID?.primaryEmailID
        return email && isValidEmail(email.trim().toLowerCase())
      })
      .forEach((rec) => {
        const email = rec.emailID!.primaryEmailID!.trim().toLowerCase()
        ...
        if (!emailMap.has(email)) {
          ...
          const label = fullName || email // Fallback to email if no name exists
