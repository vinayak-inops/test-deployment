...
export function StepIndicator({
  ...
  const isStepAccessible = (stepNumber: number): boolean => {
    // Always allow staying on current step
    if (stepNumber === currentStep) {
      return true
    }

    // Allow navigating to any completed step (going back)
    ...
    // Check if all steps before the target step are completed
    for (let i = 0; i < targetIndex; i++) {
      const previousStepNumber = steps[i].number
      if (!completedSteps.includes(previousStepNumber)) {
        return false
      }
    }
          ...
          <div className="flex flex-col items-center">
            ...
            {steps.map((step, index) => {
              ...
              const isLineCompleted = Boolean(
                nextStep && completedSteps.includes(nextStep.number)
              )
