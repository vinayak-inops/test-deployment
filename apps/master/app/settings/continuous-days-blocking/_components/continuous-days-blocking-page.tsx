...
import ContinuousDaysBlockingController, { type ContinuousBlockingFormValues } from "./continuous-blocking-task-controller"

import {
  useContinuousDaysBlockingLogic,
  FIELD_LABELS,
  type ContinuousDaysBlockingSearchField,
} from "./hooks/Usecontinuousdaysblockinglogic"
import ContinuousDaysBlockingTable from "./continuous-days-blocking-table"

// ─── Page ─────────────────────────────────────────────────────────────────────

export default function ContinuousDaysBlockingPage() {
  const {
    rows,
    totalCount,
    totalPages,
    ...
    refreshData,
    navigateToAdd,
    closeForm, // Make sure this is exported from the hook
  } = useContinuousDaysBlockingLogic()
  
  const tenantCode = useGetTenantCode()
  const { employeeId } = useKeyclockRoleInfo()
  
  // Find the selected row from the rows array
  const selectedRow = useMemo(() => {
    if (!rowId || rows.length === 0) return null
    return rows.find((r) => String(r._id) === String(rowId))
  }, [rows, rowId])

  const { post } = usePostRequest<any>({
    url: "continuousDaysBlocking",
    onSuccess: () => {
      toast.success(`Config ${mode === "edit" ? "updated" : "created"} successfully`)
      void refreshData?.()
      closeForm() // Use closeForm instead of router.push
    },
    onError: () => {
  ...
  const handleFormSubmit = async (formData: ContinuousBlockingFormValues) => {
    ...
    await post({
      ...
      collectionName: "continuousDaysBlocking",
      data: payloadData,
    })
  }
  ...
  return (
    <div>
      ...
      <TableHeader
        ...
        description={
          loading || countLoading
            ? "Continuous Days Blocking Management | Loading..."
            : `Continuous Days Blocking Management | ${totalCount} configs`
        }
        canAdd={addMode}
          ...
          <div className="flex bg-muted/50 rounded-lg border flex-1">
            ...
            <div className="flex items-center bg-background border-r rounded-l-lg px-3 py-2 w-52">
              <Select
                value={searchField}
                onValueChange={(v) =>
                  setSearchField(v as ContinuousDaysBlockingSearchField)
                }
              >
                ...
                <SelectContent>
                  <SelectItem value="subsidiaries.subsidiaryCode">Subsidiary Code</SelectItem>
                  <SelectItem value="tenantCode">Tenant Code</SelectItem>
      ...
      {isFormMode && mode && (
        <>
          {mode === "add" && (
            <ContinuousDaysBlockingController
              isOpen
              mode="add"
