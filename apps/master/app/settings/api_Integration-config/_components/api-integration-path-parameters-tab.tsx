...
import type { RequestBodyField } from "./api-integration-request-body-fields-tab"
...
export default function ApiIntegrationPathParametersTab({
  values,
  onChange,
  showErrors = false,
  errorMessage,
  ...
  const hasNoPathParameters = values.length === 0

  const searchFields = useMemo<ActionTableSearchField<RequestBodyField>[]>(() => [
    { value: "fieldName", label: "Field Name", getValue: (row) => row.fieldName },
    { value: "internalField", label: "Internal Field", getValue: (row) => row.internalField },
  ], [])
  ...
  const openEdit = (index: number) => {
    ...
    setInternalField(item.internalField)
    setInternalDataType(item.internalDataType)
        ...
        <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-[13px] font-semibold text-gray-900 leading-none">
              Path Parameters ({values.length})
            </h2>
            <p className="text-[11px] text-gray-500 mt-0.5 leading-snug">
              Configure path parameter mappings for the API endpoint.
            </p>
          </div>
        </div>
        <div className="px-6 py-4">
          <div className="relative">
                ...
                <Card className="w-full max-h-[80vh] flex flex-col overflow-hidden border border-gray-200 shadow-lg">
                  <CardHeader className="shrink-0 px-6 py-3 border-b border-gray-200 bg-white">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-base font-semibold text-gray-700">
                        {editIndex !== null ? "Edit Path Parameter" : "Add Path Parameter"}
                      </CardTitle>
                      <button 
                        type="button" 
                        onClick={closeEditor} 
                  ...
                  <CardContent className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
                    ...
                    <div className="space-y-2">
                      ...
                      <label htmlFor="required" className="inline-flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                        ...
                        />
                        <span>Mark this parameter as required</span>
                      </label>
                    </div>
                  </CardContent>
                  <CardFooter className="shrink-0 px-6 py-3 border-t border-gray-200 justify-end bg-white gap-2">
                    ...
                    <Button 
                      ...
                      onClick={handleSavePathParameter} 
                      disabled={!fieldName.trim() || !internalField.trim()}
                    >
                      {editIndex !== null ? "Save Changes" : "Add Parameter"}
                    </Button>
                  </CardFooter>
                </Card>
              </div>
            ...
            <ActionDataTable<RequestBodyField>
              ...
              emptyTitle="No path parameters configured"
              emptyDescription='Use "Add Path Parameter" to create one.'
      ...
      <div className="sticky bottom-0 w-full bg-white border-t border-gray-200 shadow-lg p-6 z-50 mt-auto">
        <div className="max-w-3xl mx-auto">
          <div className="w-[71%] mx-auto space-y-3">
            {showErrors && hasNoPathParameters && (
              <div className="bg-red-50 border border-red-200 rounded-lg px-3 py-2">
                <div className="flex items-center gap-2">
                  <AlertTriangle className="h-4 w-4 text-red-600 flex-shrink-0" />
                  <p className="text-xs font-medium text-red-900">
                    {errorMessage || "Add at least one path parameter to continue"}
                  </p>
                </div>
              </div>
            )}
            {onContinue && (
              <Button 
                type="button" 
                className="w-full h-10 bg-blue-600 hover:bg-blue-700 text-white disabled:bg-gray-300 disabled:cursor-not-allowed" 
                onClick={onContinue}
                disabled={hasNoPathParameters || isSubmitting || viewMode}
              >
