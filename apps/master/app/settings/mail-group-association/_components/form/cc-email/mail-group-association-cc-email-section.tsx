...
import MailGroupAssociationCcSelectorPopup from "./mail-group-association-cc-selector-popup"
import MailGroupAssociationCcSelectedTable from "./mail-group-association-cc-selected-table"
...
export default function MailGroupAssociationCcEmailSection({ mode, recordId }: Props) {
  ...
  const selectedRows = useMemo(() => ccEmails.map((email) => ({ email })), [ccEmails])
  ...
  return (
    <div className="space-y-4">
      <MailGroupAssociationCcSelectedTable
      ...
      <MailGroupAssociationCcSelectorPopup
        open={isSelectorOpen && !viewMode}
        onClose={() => setIsSelectorOpen(false)}
        selectedEmailIds={ccEmails}
        excludedEmailIds={primaryEmails}
        onSelectedEmailIdsChange={setCcEmails}
      />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white border border-red-300 rounded-lg shadow-xl max-w-md w-full mx-4">
            <div className="px-5 py-4 border-b border-red-100 flex items-center gap-3 bg-red-50 rounded-t-lg">
              ...
              <div>
                <h3 className="text-sm font-semibold text-red-900">Remove CC Email</h3>
                <p className="text-[11px] text-red-600 mt-0.5">Are you sure you want to remove this email?</p>
              </div>
            </div>
            <div className="px-5 py-4 space-y-3">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  type="button"
                  onClick={async () => {
                    const updatedEmails = ccEmails.filter((e) => e !== deleteTarget)
                    const payload = {
                      tenant: tenantCode,
