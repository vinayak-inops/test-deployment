...
const extractPrimaryEmail = (
  emailID: string | { primaryEmailID?: string; secondaryEmailID?: string } | null,
): string => {
  if (!emailID) return ""
...
export default function MailGroupAssociationCcSelectorPopup({
  open,
  onClose,
  selectedEmailIds,
  excludedEmailIds,
  ...
  const { post: postAssociation, loading: saving } = usePostRequest<any>({
    ...
    onSuccess: () => {
      ...
      toast.success(isEditMode ? "CC emails updated successfully." : "CC emails saved successfully.")
      onClose()
    },
  })
  ...
  const allFetchedEmails = ((data?.fetchCompanyEmployee || []) as Array<{
    emailID: string | { primaryEmailID?: string; secondaryEmailID?: string } | null
    isDeleted?: boolean
  }>)
  ...
  const availableOptions = Array.from(new Set(allFetchedEmails)).filter(
    (email) =>
      !excludedEmailIds.includes(email) &&
      !selectedEmailIds.includes(email) &&
      !draftSelected.includes(email),
  )
  ...
  const addEmail = (rawValue: string) => {
    ...
    if (selectedEmailIds.includes(email) || excludedEmailIds.includes(email)) {
    ...
    { key: "email", label: "Email", render: (row) => row.email },
    {
      ...
      render: (row) => (
        <Button
          ...
          onClick={() => setDraftSelected((prev) => prev.filter((item) => item !== row.email))}
        >
      ...
      <div className="bg-white rounded-lg shadow-xl border border-gray-200 w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="px-5 py-3 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-900">Select CC Email</h3>
        ...
        <div className="px-5 py-4 overflow-y-auto flex-1 min-h-0 space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button type="button" onClick={() => addEmail(inputValue)} className="bg-blue-600 hover:bg-blue-700 text-white">
                Add Email
              </Button>
            </div>
          ...
          <SubFormTitle title={`Selected CC Emails (${draftSelected.length})`} />
          <ActionDataTable<{ email: string }>
            ...
            isViewMode={false}
            emptyTitle="No CC emails selected"
            emptyDescription="Select email from above table."
            getRowKey={(row) => `cc-popup-${row.email}`}
          />
        </div>
