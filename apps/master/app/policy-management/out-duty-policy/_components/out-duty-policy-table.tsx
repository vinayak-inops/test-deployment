...
import type { OutDutyPolicyRow } from "./hooks/useOutDutyPolicyLogic"

// ─── Types ────────────────────────────────────────────────────────────────────

interface OutDutyPolicyTableProps {
  rows: OutDutyPolicyRow[]
  loading: boolean
...
function PolicyFlags({
  ...
  sandwichWeekOff: boolean
  noticePeriod: boolean
  autoApproval: boolean
}) {
  const flags = [
    { label: "Sandwich Holiday", active: sandwichHoliday },
    { label: "Sandwich WeekOff", active: sandwichWeekOff },
    { label: "Notice Period",    active: noticePeriod },
    { label: "Auto Approval",    active: autoApproval },
  ]
  return (
...
export default function OutDutyPolicyTable({
  rows,
  loading,
  countLoading,
  totalCount,
    ...
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          Loading out-duty policy records...
        </p>
      </div>
    )
  }
  ...
  if (rows.length === 0) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          No out-duty policy records found
        </p>
      </div>
    )
  }
          ...
          <TableBody>
            {rows.map((row, index) => (
              ...
              >
                ...
                <TableCell className="px-4 py-2">
                  <div className="flex flex-col">
                    <span className="text-sm text-gray-800">{row.designationName}</span>
                    <span className="text-xs text-gray-400">{row.designationCode}</span>
                  </div>
                </TableCell>
                ...
                <TableCell className="px-4 py-2">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-xs text-gray-600">
                      Min: <span className="font-medium text-gray-800">{row.minimumMinutesPerApplication}</span>
                    </span>
                    <span className="text-xs text-gray-600">
                      Max: <span className="font-medium text-gray-800">{row.maximumMinutesPerApplication}</span>
                    </span>
                  </div>
                </TableCell>
                ...
                <TableCell className="px-4 py-2">
                  <PolicyFlags
                    sandwichHoliday={row.sandwichHolidayAsOutDuty}
                    sandwichWeekOff={row.sandwichWeekOffAsOutDuty}
                    noticePeriod={row.isAllowedInNoticePeriod}
                    autoApproval={row.autoApprovalAllowed}
                  />
                </TableCell>
