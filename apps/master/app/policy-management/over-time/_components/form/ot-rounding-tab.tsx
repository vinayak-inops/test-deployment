...
import OtRoundingFormPopup, { type OtRoundingItem } from "./ot-rounding-form-popup"
import { useRequest } from "@repo/ui/hooks/api/useGetRequest"
import { useGetTenantCode } from "@/hooks/api/search/useGetTenantCode"

type Props = {
  value: OtRoundingItem[]
  isViewMode: boolean
  error?: string
  onChange: (rows: OtRoundingItem[]) => void
  mode?: "add" | "edit" | "view"
  rowId?: string | null
  tenantCode?: string
}

export default function OtRoundingTab({
  value,
  isViewMode,
  error,
  onChange,
  ...
  const [rows, setRows] = useState<OtRoundingItem[]>(() => Array.isArray(value) ? value : [])
  ...
  const { loading, refetch } = useRequest<any[]>({
    ...
    onSuccess: (data: any[]) => {
      ...
      const rounding = Array.isArray(record?.otPolicy?.rounding) ? record.otPolicy.rounding : []
      const normalized = rounding.map((item: any, index: number) => ({
        parseID: item?.parseID || `ot-rounding-${index}`,
  ...
  const columns = useMemo<ActionTableColumn<OtRoundingItem>[]>(
    () => [
  ...
  const searchFields = useMemo<ActionTableSearchField<OtRoundingItem>[]>(
    () => [
      { value: "from", label: "From", getValue: (row) => String(row.from ?? "") },
      { value: "to", label: "To", getValue: (row) => String(row.to ?? "") },
      { value: "roundOffTo", label: "Round Off To", getValue: (row) => String(row.roundOffTo ?? "") },
    ],
    [],
  )
    ...
    <div className="relative bg-white border border-gray-200 rounded-lg shadow-sm">
      ...
      <div className="px-6 py-4 space-y-4">
        {loading ? (
          ...
          <ActionDataTable<OtRoundingItem>
            rows={rows}
            columns={columns}
