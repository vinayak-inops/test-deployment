...
export type CompOffSearchField =
  | "subsidiary.subsidiaryName"
  | "location.locationName"
  | "designation.designationName"
  | "compOffPolicy.compOffPolicyCode"
  | "compOffPolicy.compOffPolicyTitle"

export interface CompOffPolicyRow {
  _id: string
  // subsidiary
...
export const FIELD_LABELS: Record<CompOffSearchField, string> = {
  "subsidiary.subsidiaryName":          "Subsidiary",
  "location.locationName":              "Location",
  "designation.designationName":        "Designation",
  "compOffPolicy.compOffPolicyCode":    "Policy Code",
  "compOffPolicy.compOffPolicyTitle":   "Policy Title",
}

// ─── Hook ─────────────────────────────────────────────────────────────────────

export function useCompOffPolicyLogic() {
  ...
  const [rows, setRows]               = useState<CompOffPolicyRow[]>([])
  const [searchField, setSearchField] = useState<CompOffSearchField>("compOffPolicy.compOffPolicyTitle")
  ...
  const { loading: countLoading, refetch: refetchCounter } = useRequest<any>({
    ...
    onError: (e) => {
      console.error("Failed to load compoff_policy count", e)
      setTotalCount(0)
    },
  })
  ...
  const { loading, refetch } = useRequest<any[]>({
    url:    `compoff_policy/search?offset=${offset}&limit=${PAGE_SIZE}`,
    method: "POST",
    data:   apiCriteria,
    ...
    onSuccess: (data) => {
      ...
      const mapped: CompOffPolicyRow[] = list.map((item: any) => ({
        ...
        generateOnWeekDay:    item?.compOffPolicy?.generateOnWeekDay  ?? false,
        generateOnWeekOff:    item?.compOffPolicy?.generateOnWeekOff  ?? false,
        generateOnHoliday:    item?.compOffPolicy?.generateOnHoliday  ?? false,
        compOffGenerationUnit: item?.compOffPolicy?.compOffGenerationUnit || "-",
        autoApprove:          item?.compOffPolicy?.autoApprove        ?? false,
    ...
    onError: (error) => {
      console.error("Error loading compoff_policy data:", error)
      setRows([])
    },
  })
