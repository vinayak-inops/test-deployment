...
interface Props {
  ...
  roundingSlabs?: CompOffRoundingItem[]
  onRoundingLoad?: (items: CompOffRoundingItem[]) => void
}

// ─── Component ────────────────────────────────────────────────────────────────

export default function CompOffPolicyForm({
  mode,
  rowId = null,
  ...
  const { loading: recordLoading, refetch: fetchRecord } = useRequest<any[]>({
      ...
      if (record) {
        setFormData({
          ...
          employeeCategory: Array.isArray(record?.employeeCategory)
            ...
            : [],
          compOffPolicy: {
            compOffPolicyCode:  record?.compOffPolicy?.compOffPolicyCode  || "",
            compOffPolicyTitle: record?.compOffPolicy?.compOffPolicyTitle || "",
            generateOnWeekDay:          record?.compOffPolicy?.generateOnWeekDay          ?? false,
            generateOnWeekOff:          record?.compOffPolicy?.generateOnWeekOff          ?? false,
            generateOnHoliday:          record?.compOffPolicy?.generateOnHoliday          ?? false,
            halfDayCompOffApplicable:   record?.compOffPolicy?.halfDayCompOffApplicable   ?? false,
            expireCompOffAtYearEnd:     record?.compOffPolicy?.expireCompOffAtYearEnd     ?? false,
            backDateCompOffAllowed:     record?.compOffPolicy?.backDateCompOffAllowed      ?? false,
            allowDuringNoticePeriod:    record?.compOffPolicy?.allowDuringNoticePeriod    ?? false,
            deductLunchBreakForCompOff: record?.compOffPolicy?.deductLunchBreakForCompOff ?? false,
            compOffApplicationRequired: record?.compOffPolicy?.compOffApplicationRequired ?? false,
            autoApprove:                record?.compOffPolicy?.autoApprove                ?? false,
            compOffExpiryDays:               record?.compOffPolicy?.compOffExpiryDays               ?? 0,
            maxBackDaysAllowed:              record?.compOffPolicy?.maxBackDaysAllowed              ?? 0,
            compOffMonthlyLimit:             record?.compOffPolicy?.compOffMonthlyLimit             ?? 0,
            minimumMinutesToGetFullCompOff:  record?.compOffPolicy?.minimumMinutesToGetFullCompOff  ?? 0,
            minimumMinutesToGetHalfCompOff:  record?.compOffPolicy?.minimumMinutesToGetHalfCompOff  ?? 0,
            multiplierForWeekDay:            record?.compOffPolicy?.multiplierForWeekDay            ?? 1,
            multiplierForWeekOff:            record?.compOffPolicy?.multiplierForWeekOff            ?? 1,
            multiplierForHoliday:            record?.compOffPolicy?.multiplierForHoliday            ?? 1,
            daysUntilAutoApproval:           record?.compOffPolicy?.daysUntilAutoApproval           ?? 0,
            compOffGenerationUnit:           record?.compOffPolicy?.compOffGenerationUnit           || "days",
            forHolidayAndWeekOffOverlap: {
              multiplierWithoutWorking: record?.compOffPolicy?.forHolidayAndWeekOffOverlap?.multiplierWithoutWorking ?? 0,
              multiplierForWorking:     record?.compOffPolicy?.forHolidayAndWeekOffOverlap?.multiplierForWorking     ?? 0,
            },
            cannotCombineWith: {
              prefix:  record?.compOffPolicy?.cannotCombineWith?.prefix  || [],
              postfix: record?.compOffPolicy?.cannotCombineWith?.postfix || [],
            },
            rounding: record?.compOffPolicy?.rounding || [],
          },
        })
        onRoundingLoad?.(Array.isArray(record?.compOffPolicy?.rounding) ? record.compOffPolicy.rounding : [])
      } else if (rowId) {
    ...
    onError: () => {
      toast.error("Failed to load comp-off policy data")
      setFetchingData(false)
      onClose()
    },
  })
  ...
  }, [subsidiariesArray, locationsArray, designationsArray, employeeCategoriesArray])
  ...
  const submit = async () => {
    ...
    setShowErrors(true)
    const parsed = compOffPolicyFormSchema.safeParse(formData)
    if (!parsed.success) {
      const nextErrors: Record<string, string> = {}
  ...
  const getTitle = () => {
    ...
    return "Add Comp-Off Policy"
  }

  // Determine if Policy Code should be disabled (disabled in edit and view modes)
  const isPolicyCodeDisabled = isViewMode || isEditMode
  ...
  return (
    <div className="w-full space-y-6">
      <Card className="w-full mx-auto border border-gray-200 bg-white shadow-sm">
        <GradientFormHeader
          icon={Settings2}
          title={`Comp-Off Policy - ${getTitle()}`}
          description="Define comp-off policy settings including generation rules, multipliers, limits, and employee categories"
        />

        <CardContent className="px-6 py-4 space-y-6">
          {(isOrgLoading || isLoadingData) && (
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  {/* Policy Code - Disabled in edit and view modes */}
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className={`${fieldStyles} bg-white ${showErrors && errors["compOffPolicy.compOffPolicyCode"] ? "border-red-500" : ""} ${isPolicyCodeDisabled ? "bg-gray-50 cursor-not-allowed" : ""}`}
                      placeholder="e.g. COMPOFF-001"
                      disabled={isPolicyCodeDisabled}
                    />
