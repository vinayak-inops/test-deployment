...
export type CompOffRoundingItem = {
  parseID?: string
  from: number
  to: number
  roundOffTo: number
...
type CompOffRoundingFormField = "from" | "to" | "roundOffTo"
...
const FIELDS: { name: CompOffRoundingFormField; label: string }[] = [
  { name: "from", label: "From" },
  { name: "to", label: "To" },
  { name: "roundOffTo", label: "Round Off To" },
]

const fieldMap: Record<string, CompOffRoundingFormField> = {
  from: "from",
  to: "to",
...
type Props = {
  open: boolean
  isViewMode?: boolean
  initialItem: CompOffRoundingItem | null
  onClose: () => void
  onSubmit: (item: CompOffRoundingItem) => void
  tenantCode?: string
  rowId?: string | null
}

export default function CompOffRoundingFormPopup({
  open,
  isViewMode,
  initialItem,
  onClose,
  ...
  const pendingItemRef = useRef<CompOffRoundingItem | null>(null)
  ...
  const { post: postRounding, loading: postLoading } = usePostRequest<any>({
    ...
    onError: () => {
      toast.error("Comp-off rounding submission failed!")
    },
  })
  ...
  const handleFormSubmit = (data: FormData) => {
    clearErrors()
    const item: CompOffRoundingItem = {
    ...
    postRounding({
      ...
      event: "validate",
      id: rowId,
      ruleId: "compOffPolicyRoundingValidator",
      data: {
        compOffPolicy: {
