...
import {
  useWfhPolicyLogic,
  FIELD_LABELS,
  type WfhPolicySearchField,
} from "./hooks/useWfhPolicyLogic"
import WfhPolicyTable from "./wfh-policy-table"
import WfhPolicyAddPopup from "./wfh-policy-add-popup"
      ...
      <TableHeader
        ...
        description={
          loading || countLoading
            ? "WFH Policy | Loading..."
            : `WFH Policy | ${totalCount} records`
        }
        canAdd={addMode}
        onAddNew={() => setIsAddPopupOpen(true)}
        addButtonText="Add WFH Policy"
      />
        ...
        <div className="max-w-7xl mx-auto flex items-center gap-4">
          <div className="flex bg-muted/50 rounded-lg border flex-1">
            <div className="flex items-center bg-background border-r rounded-l-lg px-3 py-2 w-52">
              <Select
                value={searchField}
                onValueChange={(v) => setSearchField(v as WfhPolicySearchField)}
              >
                <SelectTrigger className="w-full h-6 border-none p-0 text-sm font-medium text-foreground focus:ring-0 bg-transparent shadow-none">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="wfhPolicy.wfhPolicyTitle">Policy Title</SelectItem>
                  <SelectItem value="wfhPolicy.wfhPolicyCode">Policy Code</SelectItem>
