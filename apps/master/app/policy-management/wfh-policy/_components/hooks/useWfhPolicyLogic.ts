...
export type WfhPolicySearchField =
  | "subsidiary.subsidiaryName"
  | "location.locationName"
  | "designation.designationName"
  | "wfhPolicy.wfhPolicyCode"
  | "wfhPolicy.wfhPolicyTitle"

export interface WfhPolicyRow {
  _id: string
  // subsidiary
  ...
  // wfh policy detail
  wfhPolicyCode:         string
  wfhPolicyTitle:        string
...
export const FIELD_LABELS: Record<WfhPolicySearchField, string> = {
  "subsidiary.subsidiaryName":    "Subsidiary",
  "location.locationName":        "Location",
  "designation.designationName":  "Designation",
  "wfhPolicy.wfhPolicyCode":      "Policy Code",
  "wfhPolicy.wfhPolicyTitle":     "Policy Title",
}

// ─── Hook ─────────────────────────────────────────────────────────────────────

export function useWfhPolicyLogic() {
  ...
  const [rows, setRows]               = useState<WfhPolicyRow[]>([])
  const [searchField, setSearchField] = useState<WfhPolicySearchField>("wfhPolicy.wfhPolicyTitle")
  ...
  const { loading: countLoading, refetch: refetchCounter } = useRequest<any>({
    ...
    onError: (e) => {
      console.error("Failed to load WFH policy count", e)
      setTotalCount(0)
    },
  })
  ...
  const { loading, refetch } = useRequest<any[]>({
    url:    `wfh_policy/search?offset=${offset}&limit=${PAGE_SIZE}`,
    method: "POST",
    data:   apiCriteria,
    ...
    onSuccess: (data) => {
      ...
      const mapped: WfhPolicyRow[] = list.map((item: any) => {
        const p = item?.wfhPolicy || {}
        return {
    ...
    onError: (error) => {
      console.error("Error loading WFH policy data:", error)
      setRows([])
    },
  })
