...
import ShiftsForm from "./shift-list/[...shift-list]/_component/shifts-form";
...
function ShiftListContent() {
  ...
  const attendanceCriteria = useMemo(() => {
    ...
    const subsidiaryList = Array.isArray(userEntitlement?.subsidiary) && userEntitlement.subsidiary.length > 0
      ? userEntitlement.subsidiary
      : Array.isArray(hierarchyFilters?.subsidiaries)
        ? hierarchyFilters.subsidiaries
        : []

    const locationList = Array.isArray(userEntitlement?.location) && userEntitlement.location.length > 0
    ...
    return criteria
  }, [tenantCode, userEntitlement, hierarchyFilters])
    ...
    if (normalizedSearch) {
      data = data.filter((item: any) => {
        const fieldValueMap: Record<string, string> = {
          shiftGroupCode: String(item?.shiftGroupCode ?? ""),
          shiftGroupName: String(item?.shiftGroupName ?? ""),
          subsidiaryName: String(item?.subsidiary?.subsidiaryName ?? ""),
          locationName: String(item?.location?.locationName ?? ""),
        };
  ...
  const existingShiftGroupCodes = attendanceResponse
    ? attendanceResponse
        .map((item: any) => String(item?.shiftGroupCode ?? "").toLowerCase())
        .filter(Boolean)
    : [];
  const existingShiftGroupNames = attendanceResponse
    ? attendanceResponse
        .map((item: any) => String(item?.shiftGroupName ?? "").toLowerCase())
        .filter(Boolean)
    : [];
