...
import {
  createPersonalDashboardPermissionSchema,
  type PersonalDashboardPermissionPayload,
} from "./personal-dashboard-permission-schema"

type PersonalDashboardPermissionState = {
  view: boolean
}

interface PersonalDashboardFormProps {
  initialData?: PersonalDashboardPermissionPayload | PersonalDashboardPermissionState
  onSave?: (data: PersonalDashboardPermissionPayload) => void
  mode?: "add" | "edit" | "view"
}
...
type PersonalDashboardFormStructure = {
  label?: string
  title?: string
  subtitle?: string
  description?: string
...
export default function PersonalDashboardForm({
  initialData,
  onSave,
  mode = "add",
}: PersonalDashboardFormProps) {
  ...
  const resolvedFormStructure = useMemo(() => {
    ...
    if (!root) return {} as PersonalDashboardFormStructure

    const candidate =
      pickFirstObject(
        root.personalDashboard,
        getObjectValue(root.setting)?.personalDashboard,
      ) || root

    return candidate as PersonalDashboardFormStructure
  }, [formStructure])

  const personalDashboardConfig =
    resolvedFormStructure.fields?.personaldashboard || resolvedFormStructure.personaldashboard
  const fields =
    resolvedFormStructure.fields?.personaldashboard?.fields?.permissions?.fields ||
    resolvedFormStructure.personaldashboard?.fields?.permissions?.fields ||
    {}
  const hasConfig = Boolean(personalDashboardConfig)
  const getFieldLabel = (key: keyof PersonalDashboardPermissionState, fallback: string) =>
    fields[key]?.label || fallback
  const isFieldVisible = (key: keyof PersonalDashboardPermissionState) =>
    hasConfig && !!fields[key] && fields[key]?.visible === true

  const { post: postPersonalDashboardPermissions, loading: postLoading } = usePostRequest<any>({
    url: "role_permissions",
    onSuccess: () => {
      toast.success("Personal dashboard permissions saved successfully!")
    },
    onError: (error) => {
      console.error("Error saving personal dashboard permissions:", error)
      toast.error("Failed to save personal dashboard permissions!")
    },
  })
  ...
  const [fetchedPayload, setFetchedPayload] = useState<PersonalDashboardPermissionPayload | null>(
    null
  )
  const [fetchedRecordId, setFetchedRecordId] = useState<string | null>(null)

  const { refetch: refetchPersonalDashboard } = useRequest<any>({
    ...
    onSuccess: (response) => {
      ...
      if (raw?.dashboard && typeof raw.dashboard === "object") {
        setFetchedPayload(raw as PersonalDashboardPermissionPayload)
        return
      }
    ...
    onError: (error) => {
      console.error("Error fetching personal dashboard permissions:", error)
    },
  })

  const buildStateFromInitial = (
    data?: PersonalDashboardPermissionPayload | PersonalDashboardPermissionState | null
  ): PersonalDashboardPermissionState => {
    const base: PersonalDashboardPermissionState = {
      view: false,
    }
    ...
    if ("dashboard" in data && data.dashboard) {
      const personaldashboard = data.dashboard.personaldashboard

      return {
        ...base,
        view: !!personaldashboard?.permissions?.view,
      }
    }

    return {
      ...base,
      ...(data as PersonalDashboardPermissionState),
    }
  }

  const sourceData = useMemo(() => {
    if (fetchedPayload) return fetchedPayload
    if (initialData && "dashboard" in initialData) return initialData
    return initialData || null
  }, [fetchedPayload, initialData])

  const [permissions, setPermissions] = useState<PersonalDashboardPermissionState>(() =>
  ...
  const handlePermissionChange = (key: keyof PersonalDashboardPermissionState, value: boolean) => {
    setPermissions((prev) => ({
      ...prev,
      [key]: value,
    }))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload: PersonalDashboardPermissionPayload = {
        dashboard: {
          isActive,
          personaldashboard: {
            isActive,
      ...
      const schema = createPersonalDashboardPermissionSchema()
      const validated = schema.safeParse(payload)

      if (!validated.success) {
        toast.error("Invalid personal dashboard payload")
        return
      }
      ...
      const requestPayload = {
        ...
        data: {
          ...
          dashboard: {
            personaldashboard: validated.data.dashboard.personaldashboard,
          },
        },
      }

      await postPersonalDashboardPermissions(requestPayload)
      await refetchPersonalDashboard?.()
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <RolePermissionConfirmModal
        ...
        title={resolvedFormStructure.confirmTitle || "Confirm Personal Dashboard Permission Change"}
        description={
          resolvedFormStructure.confirmDescription ||
          "You are updating personal dashboard permissions. This can impact user access."
        }
        body={
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-gray-900">
                {resolvedFormStructure.label || "Personal Dashboard Permissions"}
              </h2>
              <p className="text-[11px] text-gray-500 mt-0.5">
                {resolvedFormStructure.description ||
                  resolvedFormStructure.subtitle ||
                  "Manage personal dashboard view permissions and access controls"}
              </p>
            </div>
          </div>
        </div>
                  ...
                  {isFieldVisible("view") && (
                    <div className="group relative flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:border-gray-300 transition-colors">
                      <div className="flex-1 pr-4">
                        ...
                        >
                          Allow viewing personal dashboard
                        </label>
                      </div>
                      <SwitchToggle
                        id="view"
