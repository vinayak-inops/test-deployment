...
export default function SecurityPassPage() {
    ...
    const [selectedField, setSelectedField] = useState('contractorCode')
    ...
    const {
        ...
        onError: (error) => {
          toast.error("Contractor deletion failed!");
    ...
    const apiCriteria = useMemo(() => {
        ...
        if (debouncedSearchTerm.trim() && filtersApplied) {
            ...
            if (selectedField === 'contractorCode') {
                filters.push({
                    field: 'documents.contractorCode',
                    operator: "like",
                    value: searchValue
                })
    ...
    const countRequestData = useMemo(() => {
        if (!filtersApplied) {
            return [
                ...
                { field: "collectionName", operator: "eq", value: "contractor" },
        ...
        onSuccess: (data) => {
            if (data !== null && data !== undefined) {
                ...
                const getDocuments = (item: any) =>
                    Array.isArray(item?.documents)
                        ...
                        : Array.isArray(item?.data?.documents)
                          ? item.data.documents
                          : [];
                const joinDocField = (docs: any[], key: "contractorCode" | "contractorName") =>
                    docs.map((d: any) => d?.[key]).filter(Boolean).join(", ");
                
                const filteredData = active.map((item: any) => ({
                    _id: item._id,
                    employeeID: joinDocField(getDocuments(item), "contractorCode") || item?.contractorCode || item?.data?.contractorCode || item?.data?.ContractorCode?.[0] || item?.ContractorCode?.[0] || "",
                    ...
                    contractorCode: joinDocField(getDocuments(item), "contractorCode") || item?.deployment?.contractor?.contractorCode || item?.data?.deployment?.contractor?.contractorCode || "",
                ...
                const transformedData = active.map((item: any) => ({
                    _id: item._id,
                    contractorCode:
                        joinDocField(getDocuments(item), "contractorCode") ||
                        item?.contractorCode ||
                        item?.data?.contractorCode ||
                        (Array.isArray(item?.data?.ContractorCode) ? item.data.ContractorCode.join(", ") : item?.data?.ContractorCode) ||
                        (Array.isArray(item?.ContractorCode) ? item.ContractorCode.join(", ") : item?.ContractorCode) ||
                        "",
                    employeeID:
                        joinDocField(getDocuments(item), "contractorCode") ||
                        item?.contractorCode ||
    ...
    const prevSelectedFieldRef = useRef<string>('contractorCode')
    ...
    if (isAllMode) {
        ...
        return (
            <ContractEmployeeApproveApplicationDetail
                ...
                onBack={() => router.push("/application-management/contractor-approve")}
                onOpenApplication={(documentId) => {
    ...
    return (
        <>
        {
            ...
            <SidebarFromHeader
                ...
                description="Manage and review contractor applications for HR approval."
                canAdd={false}
                onOpenDraftList={() => setShowTableFilterPopup(true)}
                draftButtonText="Select Contractors"
            />
                    ...
                    onFormView={(employee) => {
                        const employeeIdValue =
                            typeof employee._id === "object" && employee._id && '$oid' in employee._id
                                ...
                                : String(employee._id || "")
                        router.push(`/application-management/contractor-approve?mode=all&id=${employeeIdValue}`)
                    }}
                />
            )}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => { if (!deleteLoading) handleCloseDelete() }}>
                    <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="mb-4">
                            ...
                            Are you sure you want to delete this contractor? This will permanently remove the record from the system.
                        </p>
                            {deleteLoading && (
            ...
            {showVerificationPopup && (
                ...
                >
                    ...
                    >
                        ...
                        <Card className="w-full h-[88vh] max-h-[90vh] flex flex-col overflow-hidden">
                            <CardHeader className="px-6 py-3 border-b border-gray-200">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex items-center gap-1.5 px-2 py-0.5 bg-blue-50 text-blue-700 text-xs font-medium rounded-full border border-blue-100">
            ...
            <ContractEmployeeApproveTableFilterPopup
                ...
                onAction={(employee) => {
                    ...
                    router.push(`/application-management/contractor-approve?form=temp&mode=view&id=${encryptedData}&returnTo=${encodeURIComponent("/application-management/contractor-approve?popup=1")}`)
                }}
            />
        </div>
                </>
