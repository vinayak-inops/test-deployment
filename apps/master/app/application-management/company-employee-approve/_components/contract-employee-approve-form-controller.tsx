...
import { EmployeeDeploymentFormContent, type Mode } from '@/app/company_management/company-employee/_components/employee-deployment-form-content'
...
function FormController({ encryptedId: encryptedIdProp, onBack }: ContractEmployeeApproveFormControllerProps) {
  ...
  const employeeCollectionUrl = isDraftForm ? "draft/company_employee" : "company_employee"
  ...
  const { data: employeeResponse, loading: isLoadingEmployee } = useRequest<any>({
  ...
  return (
    <div className="w-full mx-auto">
      <EmployeeShiftHeader
        ...
        showBackButton={true}
        onBack={onBack || (() => router.push(returnTo || "/application-management/company-employee-approve"))}
        canAdd={false}
      />
