...
export default function ContractEmployeeApproveTableFilterPopup({
  ...
  const apiCriteria = useMemo(() => {
    ...
    if (query) {
      if (searchField === "designationCode") {
        filters.push({ field: "deployment.designation.designationCode", operator: "like", value: query });
      } else {
  ...
  const { loading, refetch: refetchSecurityPass } = useRequest<any>({
    url: `draft/company_employee/search?offset=${offset}&limit=${itemsPerPage}`,
    method: "POST",
    data: apiCriteria,
    onSuccess: (data: any) => {
      ...
      tableRows.forEach((row) => {
        ...
        if (checked) {
          next.set(id, {
            ...
            employeeName: getFullName(row) || "",
            designationCode: row.deployment?.designation?.designationCode || "",
          });
        } else {
          next.delete(id);
        }
    ...
    setSelectedEmployeeMap((prev) => {
      ...
      if (checked) {
        next.set(id, {
          _id: id,
          employeeID: row?.employeeID || "",
          employeeName: row ? getFullName(row) : "",
          designationCode: row?.deployment?.designation?.designationCode || "",
        });
      } else {
        next.delete(id);
  ...
  const handleFinalSubmit = async () => {
    ...
    const documents = Array.from(selectedIds)
      ...
      .filter((doc): doc is SelectedDocumentRow =>
        Boolean(doc && doc._id && doc.employeeID)
      );
    if (documents.length === 0) {
      toast.error("No valid employee records selected.");
      return;
    }
                ...
                <div className="flex items-center bg-background border-r rounded-l-lg px-3 py-2 w-44">
                  ...
                  <Select value={searchField} onValueChange={setSearchField}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="designationCode">Designation Code</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              ...
              <table className="min-w-[1200px] w-full border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-600 whitespace-nowrap sticky top-0 z-10 bg-gray-50">Designation</th>
                ...
                <tbody>
                  {loading ? (
                    <tr>
                      <td colSpan={6} className="px-3 py-8 text-center text-sm text-gray-500">Loading...</td>
                    </tr>
                  ) : tableRows.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-3 py-8 text-center text-sm text-gray-500">No records found</td>
                    </tr>
                  ) : (
                    tableRows.map((employee, index) => (
                      <tr key={employee._id || `${employee.employeeID}-${index}`} className="border-b border-gray-100">
                        ...
                        <td className="px-3 py-2 text-sm text-gray-700 whitespace-nowrap max-w-[280px] truncate">
                          {employee.deployment?.designation?.designationCode || "-"}
                        </td>
