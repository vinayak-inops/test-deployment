...
export default function SecurityPassPage() {
    ...
    const apiCriteria = useMemo(() => {
        ...
        if (debouncedSearchTerm.trim() && filtersApplied) {
            ...
            } else if (selectedField === 'contractorCode') {
                filters.push({
    ...
    const countRequestData = useMemo(() => {
        if (!filtersApplied) {
            return [
                ...
                { field: "collectionName", operator: "eq", value: "contract_employee" },
            ...
            if (data !== null && data !== undefined) {
                ...
                const getDocuments = (item: any) =>
                    Array.isArray(item?.documents)
                        ...
                        : Array.isArray(item?.data?.documents)
                          ? item.data.documents
                          : [];
                const joinDocField = (docs: any[], key: "employeeID" | "contractorCode" | "contractorName") =>
                ...
                const filteredData = active.map((item: any) => {
                    ...
                    return {
                        ...
                        contractorCode:
                            joinDocField(docs, "contractorCode") ||
                            item?.deployment?.contractor?.contractorCode ||
                            item?.data?.deployment?.contractor?.contractorCode ||
                            "",
                        contractorName:
                            joinDocField(docs, "contractorName") ||
                            item?.deployment?.contractor?.contractorName ||
                            item?.data?.deployment?.contractor?.contractorName ||
                            "",
                ...
                const transformedData = active.map((item: any) => {
                    ...
                    return {
                        _id: item._id,
                        contractorCode: joinDocField(docs, "contractorCode") || "",
                        contractorName: joinDocField(docs, "contractorName") || "",
                        employeeID:
                            joinDocField(docs, "employeeID") ||
                            item?.employeeID ||
                            item?.data?.employeeID ||
    ...
    if (isAllMode) {
        ...
        return (
            <ContractEmployeeApproveApplicationDetail
                ...
                onBack={() => router.push("/application-management/contract-employee-approve")}
    ...
    return (
        <>
        {
            ...
            <SidebarFromHeader
                ...
                description="Manage and review employee applications for HR approval."
                canAdd={false}
                    ...
                    onFormView={(employee) => {
                        const employeeIdValue =
                            typeof employee._id === "object" && employee._id && '$oid' in employee._id
                                ...
                                : String(employee._id || "")
                        router.push(`/application-management/contract-employee-approve?mode=all&id=${employeeIdValue}`)
                    }}
                />
            )}
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <CardTitle className="text-base font-semibold text-gray-700">
                                            Contract Employee Verification
                                        </CardTitle>
            ...
            <ContractEmployeeApproveTableFilterPopup
                ...
                onAction={(employee) => {
                    ...
                    const returnTo = encodeURIComponent("/application-management/contract-employee-approve?popup=1")
                    router.push(`/application-management/contract-employee-approve?form=temp&mode=view&id=${encryptedData}&returnTo=${returnTo}`)
                }}
            />
        </div>
                </>
