...
function extractEmployeeFlags(rows: any[]): { isManager: boolean; isEndUser: boolean } {
  const isManager = rows.some((row) => Boolean(row?.isManager ?? row?.is_manager))
  const isEndUser = rows.some((row) => Boolean(row?.isEndUser ?? row?.is_end_user))
  return { isManager, isEndUser }
}
...
export function useEmpHierarchy(criteria?: EmployeeCriteria) {
  ...
  const { employeeIds, employeesLite, rawEmployees, isManager, isEndUser } = useMemo(() => {
    ...
    const { isManager, isEndUser } = extractEmployeeFlags(filteredRaw)
    return { employeeIds: ids, employeesLite: lite, rawEmployees: filteredRaw, isManager, isEndUser }
