...
/**
 * Helper function to check if a value is "ALL" (case-insensitive)
 */
const isAllValue = (value: string | string[] | null | undefined): boolean => {
  if (!value) return false
  if (typeof value === "string") {
    return value.trim().toUpperCase() === "ALL"
  }
  if (Array.isArray(value)) {
    return value.length === 1 && value[0]?.trim().toUpperCase() === "ALL"
  }
  return false
}

/**
 * Helper function to get the first non-ALL value from hierarchy filters
 */
const getFilterValue = (value: string | string[] | null | undefined): string | undefined => {
  if (!value) return undefined
  if (isAllValue(value)) return undefined
  if (typeof value === "string") {
    return value.trim()
  }
  if (Array.isArray(value)) {
    // Filter out "ALL" values and take the first valid one
    const validValues = value.filter(v => v && v.trim().toUpperCase() !== "ALL")
    return validValues.length > 0 ? validValues[0].trim() : undefined
  }
  return undefined
}
...
export function useEmployeeShiftGraphql({
  ...
  const shiftGroupQueryVariables = useMemo(() => {
    ...
    // Helper to get first non-ALL value from array or string
    const getFirstNonAllValue = (value: string | string[] | null | undefined): string | undefined => {
      if (isAllValue(value)) return undefined
      if (!value) return undefined
      if (typeof value === "string") return value.trim()
      if (Array.isArray(value)) {
        const valid = value.find(v => v && v.trim().toUpperCase() !== "ALL")
        return valid?.trim()
      }
      return undefined
    }
    
    // Get hierarchy filter values (skip if "ALL")
    const hierarchySubsidiary = getFirstNonAllValue(hierarchyFilters?.subsidiary)
    const hierarchyLocation = getFirstNonAllValue(hierarchyFilters?.location)
    const hierarchyCategory = getFirstNonAllValue(hierarchyFilters?.categories)
    
    // Get deployment values
    const deploymentSubsidiary = employeeDeployment?.subsidiary?.subsidiaryCode
    const deploymentLocation = employeeDeployment?.location?.locationCode
    const deploymentCategory = employeeDeployment?.employeeCategory?.employeeCategoryCode
    
    // Use hierarchy filter if provided (and not "ALL"), otherwise use deployment value
    const subsidiaryValue = hierarchySubsidiary || deploymentSubsidiary
    const locationValue = hierarchyLocation || deploymentLocation
    const categoryValue = hierarchyCategory || deploymentCategory

    const criteriaRequests: Array<{ field: string; operator: string; value: any }> = [
      { field: "tenantCode", operator: "eq", value: tenantCode.trim() },
    ]
    
    // Only add filters if value exists (not "ALL" and not empty)
    if (subsidiaryValue && subsidiaryValue.trim()) {
