...
interface UseValidationPostRequestOptions<T> {
  url: string;
  data?: any;
...
interface UseValidationPostRequestResult<T> {
  data: T | null;
  loading: boolean;
...
/**
 * Custom hook for making validation POST requests with support for file uploads
 * @param options - Configuration options for the request
 * @returns Object containing data, loading state, error state, upload progress, and post function
 */
export const useValidationPostRequest = <T extends unknown>({
  url,
  data: initialData,
  files,
  headers = {},
  ...
  onError,
  onProgress
}: UseValidationPostRequestOptions<T>): UseValidationPostRequestResult<T> => {
  ...
  const post = async (postData?: any) => {
    try {
      ...
      const response = await axios({
        method: 'POST',
        url: `${API_BASE_URL}/api/workflow/attendance/validate`,
        data: requestData,
...
const { post, loading, error, data } = useValidationPostRequest<any>({
  url: 'uploadfile',
  data: {
    name: 'example',
    description: 'test'
...
// For file upload
const { post, loading, error, uploadProgress } = useValidationPostRequest<any>({
  url: 'uploadfile',
  files: selectedFile,
  headers: {
    'X-workflow': 'workflow1',
