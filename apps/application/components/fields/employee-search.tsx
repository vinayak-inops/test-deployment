...
export default function EmployeeSearchField({
  ...
  const userEntitlement = useMemo(() => {
    ...
    if (loginEmployeeId) {
      entitlement.employeeID = loginEmployeeId
    }

    // Use hierarchyFiltersFromHook to get hierarchy data for userEntitlement
    if (hierarchyFiltersFromHook) {
      if (hierarchyFiltersFromHook.subsidiaries && hierarchyFiltersFromHook.subsidiaries.length > 0) {
        entitlement.subsidiary = hierarchyFiltersFromHook.subsidiaries
      }
      if (hierarchyFiltersFromHook.divisions && hierarchyFiltersFromHook.divisions.length > 0) {
        entitlement.division = hierarchyFiltersFromHook.divisions
      }
      if (hierarchyFiltersFromHook.departments && hierarchyFiltersFromHook.departments.length > 0) {
        entitlement.department = hierarchyFiltersFromHook.departments
      }
      if (hierarchyFiltersFromHook.contractors && hierarchyFiltersFromHook.contractors.length > 0) {
        entitlement.contractor = hierarchyFiltersFromHook.contractors
      }
      if (hierarchyFiltersFromHook.locations && hierarchyFiltersFromHook.locations.length > 0) {
        entitlement.location = hierarchyFiltersFromHook.locations
      }
    }

    return Object.keys(entitlement).length > 0 ? entitlement : undefined
  }, [loginEmployeeId, hierarchyFiltersFromHook])
  ...
  const queryVariables = useMemo(() => {
    ...
    return vars
  }, [tenantCode, debouncedSearch, effectiveHierarchyFilters, userEntitlement, isOpen, organizationCode, preSelectedEmployeeId])
