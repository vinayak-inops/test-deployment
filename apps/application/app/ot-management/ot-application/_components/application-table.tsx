...
import { CheckCircle, Clock, XCircle, Calendar, User, Eye, ChevronLeft, ChevronRight, ArrowUpDown } from 'lucide-react'
import {
  Table,
  TableBody,
  TableCell,
...
export default function ApplicationTable({ data, onOpenDetails, loading = false, userMode = 'user', externalPagination, currentPage: externalCurrentPage, totalItems: externalTotalItems, itemsPerPage: externalItemsPerPage, onPageChange: externalOnPageChange }: ApplicationTableProps) {
  ...
  const currentPage = useExternal ? (typeof externalPagination === 'object' ? externalPagination.currentPage : externalCurrentPage ?? 1) : internalCurrentPage
  const totalPages = useExternal ? (typeof externalPagination === 'object' ? externalPagination.totalPages : Math.ceil((externalTotalItems ?? 0) / itemsPerPage)) : Math.ceil(data.length / itemsPerPage)
  const startIndex = useExternal ? (typeof externalPagination === 'object' ? externalPagination.startIndex : ((currentPage - 1) * itemsPerPage)) : (currentPage - 1) * itemsPerPage
  const endIndex = useExternal ? (typeof externalPagination === 'object' ? externalPagination.endIndex : Math.min(startIndex + itemsPerPage, externalTotalItems ?? 0)) : startIndex + itemsPerPage
  const totalItems = useExternal ? (typeof externalPagination === 'object' ? externalPagination.totalItems : externalTotalItems ?? 0) : data.length
  const handlePageChange = useExternal ? (typeof externalPagination === 'object' ? externalPagination.onPageChange : externalOnPageChange ?? (() => {})) : setInternalCurrentPage
  ...
  const getValue = (row: Application, key: keyof Application): any => {
    switch (key) {
      ...
      case 'duration':
        return (row.duration?.noOfDays ?? 0) * 24 + (row.duration?.noOfHours ?? 0)
      default:
  ...
  const toggleSort = (key: keyof Application) => {
    if (useExternal) return // Disable sorting when using external pagination
    if (sortKey === key) {
              ...
              <Table className="min-w-[1200px]">
                <TableHeader>
                  <TableRow className="bg-blue-50 hover:bg-blue-50">
                    ...
                    <TableHead className="w-[110px] px-5 py-2 whitespace-nowrap text-sm">Out Duty Type</TableHead>
                    ...
                    <TableHead className="w-[140px] px-5 py-2 whitespace-nowrap text-sm">Duration</TableHead>
                  ...
                  <TableHeader>
                    <TableRow className="bg-blue-50 hover:bg-blue-50">
                      ...
                      <TableHead className="w-[110px] px-5 py-2 whitespace-nowrap text-sm">
                        <div className="flex items-center justify-between gap-2">
                          <span className="inline-flex items-center gap-1">Out Duty Type</span>
                          {!useExternal && (
                            <ColumnSortMenu
                              onSortAsc={() => { setSortKey('outDutyType'); setSortDir('asc'); setInternalCurrentPage(1) }}
                              onSortDesc={() => { setSortKey('outDutyType'); setSortDir('desc'); setInternalCurrentPage(1) }}
                              onAlphaAsc={() => { setSortKey('outDutyType'); setSortDir('asc'); setInternalCurrentPage(1) }}
                              onAlphaDesc={() => { setSortKey('outDutyType'); setSortDir('desc'); setInternalCurrentPage(1) }}
                            />
                          )}
                        </div>
                      </TableHead>
                      ...
                      <TableHead className="w-[140px] px-5 py-2 whitespace-nowrap text-sm">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {!useExternal && (
                            <ColumnSortMenu
                              onSortAsc={() => { setSortKey('duration'); setSortDir('asc'); setInternalCurrentPage(1) }}
                              onSortDesc={() => { setSortKey('duration'); setSortDir('desc'); setInternalCurrentPage(1) }}
                            />
                          )}
                        </div>
                      </TableHead>
                  ...
                  <TableBody>
                    {currentData.map((row, index) => (
                      ...
                      >
                        ...
                        <TableCell className="px-5 py-2 whitespace-nowrap text-sm">{row.outDutyType || '-'}</TableCell>
                        ...
                        <TableCell className="px-5 py-2 whitespace-nowrap text-sm">{formatDuration(row.duration)}</TableCell>
