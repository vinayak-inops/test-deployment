...
export default function otRequestsPopup({ isOpen, onClose, initialSelectedRequest, selectedRequestId, isSelfPermission = false, isAllPermission = false, userMode = 'user', sourceCollectionName, onActionSuccess, modeOfRequest }: PunchRequestsPopupProps) {
  ...
  // For "user" mode: check otApplicationsSelfCancel and otApplicationsAllCancel
  // For "approver" mode: check otApplicationsCancel, otApplicationsApprove, otApplicationsReject
  ...
  const collectionName = useMemo(() => {
    ...
    // For user mode: always use regular collection (matching ot-application.tsx logic)
    // Applications, Pending, Failed, and all statuses use outDutyApplication
    return 'outDutyApplication';
  ...
  useEffect(() => {
    fetchAttendance();
  }, [selectedRequestId, collectionName]);
    ...
    >
      <div className="bg-white w-full max-w-5xl h-[85vh] flex flex-col rounded-lg shadow-2xl border border-gray-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200">
          <h2 className="text-base font-semibold text-gray-700">OT Application</h2>
              <button
                onClick={onClose}
                  ...
                  <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200">
                    ...
                    <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-semibold border ${getStatusColor(selectedRequest.workflowState)}`}>
                      ...
                      <span className="uppercase tracking-wide">{selectedRequest.workflowState || 'PENDING'}</span>
                        ...
                        {/* Format helpers */}
                        {(() => {
                          const parseDateTime = (dateStr: string) => formatDateTimeIST(dateStr)
                          ...
                          return (
                            <div className="space-y-2">
                              ...
                              {/* Out Duty Type */}
                              {selectedRequest.outDutyType && (
                                <div className="flex items-center border-b border-gray-100 pb-2">
                                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider w-32 flex-shrink-0">Out Duty Type</label>
                                  <span className="text-sm text-gray-900 font-medium">{selectedRequest.outDutyType}</span>
                                </div>
                              )}
                              ...
                              {(selectedRequest.fromDate || selectedRequest.toDate) && (
                                <div className="flex items-center border-b border-gray-100 pb-2 gap-6">
                                  <div className="flex items-center gap-2">
                                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider w-24 flex-shrink-0">From Date</label>
                                    <span className="text-sm text-gray-900 font-medium">{formatDDMMYYYY(selectedRequest.fromDate)}</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider w-20 flex-shrink-0">To Date</label>
                              ...
                              <div className="flex items-center border-b border-gray-100 pb-2">
                                ...
                                <span className="text-sm text-gray-900 font-medium">{formatDuration(selectedRequest.duration)}</span>
                              </div>
                              ...
                              {selectedRequest.Reason && (
                                <div className="flex items-start border-b border-gray-100 pb-2">
                                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider w-32 flex-shrink-0 pt-1">Reason</label>
                                  <span className="text-sm text-gray-900 font-medium flex-1">{selectedRequest.Reason}</span>
                                </div>
                              )}

                              {/* Address / Location */}
                              {selectedRequest.OutDutyAddress && (
                                <div className="flex items-start border-b border-gray-100 pb-2">
                                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider w-32 flex-shrink-0 pt-1">Address</label>
                                  <span className="text-sm text-gray-900 font-medium flex-1">{selectedRequest.OutDutyAddress}</span>
                                </div>
                              )}
                ...
                onClick={() => {
                  ...
                  setStatusLoading(true);
                    ...
                    const data: any = {
                      ...
                      approverID: approverId, // Use approverID from backend, fallback to logged-in user's employee ID
                    }
