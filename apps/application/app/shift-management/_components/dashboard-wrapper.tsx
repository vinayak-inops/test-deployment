...
import { useSelector } from "react-redux";
  ...
  import { navShiftApplication } from "@/json/menu/menu";
...
function DashboardWrapper({
  ...
  const filteredNavItems = useFilteredNavigation(navShiftApplication, adminRole);
  useEffect(() => {
    setNavList(filteredNavItems)
  }, [ filteredNavItems, navShiftApplication]);
      ...
      <div className="flex flex-wrap">
        ...
        >
          <div className="flex bg-[#f3f4f8] flex-col min-h-screen relative shadow-[0_48px_100px_0_rgba(17,12,46,0.15)]">
            {navShiftApplication?.length > 0 && (
              // <MainHeader navItems={navList}/>
              <Header navItems={navList} serviceName="Shift Change"/>
            )}
