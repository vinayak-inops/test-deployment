...
import { Edit, MoreVertical, Loader2, Gavel, ChevronLeft, ChevronRight } from "lucide-react"
...
import type { EmployeePenaltyRow } from "./hooks/Useemployeepenaltieslogic"

// ─── Types ────────────────────────────────────────────────────────────────────

interface EmployeePenaltiesTableProps {
...
function FineCell({ amount }: { amount: number | string }) {
...
function AdjustmentPeriodCell({ month, year }: { month: number | string; year: number | string }) {
  const monthName =
    typeof month === "number" && month >= 1 && month <= 12 ? MONTH_NAMES[month] : month
  return (
...
export default function EmployeePenaltiesTable({
  rows,
  loading,
  countLoading,
  totalCount,
  ...
  if (loading || countLoading) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">Loading employee penalties...</p>
      </div>
    )
  }
  ...
  if (rows.length === 0) {
    return (
      <div className="py-16 text-center">
        <Gavel className="mx-auto mb-2 h-8 w-8 text-muted-foreground" />
        <p className="text-sm text-muted-foreground">No employee penalties found</p>
      </div>
    )
  }
        ...
        <Table className="min-w-[1500px]">
          <DataTableHeader>
            <TableRow className="bg-gray-50/80 border-b border-gray-200 hover:bg-gray-50/80">
              ...
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Date of Offence</TableHead>
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Offence Description</TableHead>
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Action Taken</TableHead>
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Fine Imposed</TableHead>
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Fine Realised Date</TableHead>
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Witness</TableHead>
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Adjustment Period</TableHead>
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Cause Shown</TableHead>
          ...
          <TableBody>
            {rows.map((row, index) => (
              ...
              >
                ...
                <TableCell
                  className="px-5 py-2 text-sm max-w-[160px] truncate"
                  title={row.offenceDescription !== "-" ? row.offenceDescription : undefined}
                >
                  {row.offenceDescription}
                </TableCell>

                <TableCell
                  className="px-5 py-2 text-sm max-w-[180px] truncate"
                  title={row.actionTaken !== "-" ? row.actionTaken : undefined}
                >
                  {row.actionTaken}
                </TableCell>

                <TableCell className="px-5 py-2">
                  <FineCell amount={row.fineImposed} />
                </TableCell>

                <TableCell className="px-5 py-2 text-sm whitespace-nowrap">
                  {row.fineRealisedDate}
                </TableCell>

                {/* Witness — ID + Name stacked */}
                <TableCell className="px-5 py-2">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-xs font-medium text-gray-700">{row.witnessEmployeeID}</span>
                    {row.witnessName !== "-" && (
                      <span className="text-xs text-gray-400">{row.witnessName}</span>
                    )}
                  </div>
                </TableCell>
                ...
                <TableCell className="px-5 py-2">
                  <StatusBadge
                    value={row.isCauseShownAgainstFine}
                    trueLabel="Shown"
                    falseLabel="Not Shown"
                    trueStyle="border-purple-300 bg-purple-50 text-purple-700"
