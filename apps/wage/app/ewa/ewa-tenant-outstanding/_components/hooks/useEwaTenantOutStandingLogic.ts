...
export type EwaTenantOutstandingSearchField =
  | "month"
  | "year"
  | "paid"

export interface EwaTenantOutstandingRow {
  _id:              string
  month:            number
  year:             number
...
export const FIELD_LABELS: Record<EwaTenantOutstandingSearchField, string> = {
  month: "Month",
  year:  "Year",
  paid:  "Paid Status",
}
...
export function useEwaTenantOutstandingLogic() {
  ...
  const [rows, setRows]               = useState<EwaTenantOutstandingRow[]>([])
  const [searchField, setSearchField] = useState<EwaTenantOutstandingSearchField>("month")
  ...
  const { responseData: rolePermissions, loading: permissionsLoading } =
    useRolePermissions({
      serviceName: "ewa",
      screenName:  "ewaTenantOutstanding",
    })
  ...
  const { loading: countLoading, refetch: refetchCounter } = useRequest<any>({
    url:          "EWA_tenant_outstanding/count",
    method:       "POST",
    data:         apiCriteria,
    ...
    onError: (e) => {
      console.error("Failed to load EWA tenant outstanding count", e)
      setTotalCount(0)
    },
  })
  ...
  const { loading, refetch } = useRequest<any[]>({
    url:          `EWA_tenant_outstanding/search?offset=${offset}&limit=${PAGE_SIZE}`,
    method:       "POST",
    data:         apiCriteria,
    ...
    onSuccess: (data) => {
      ...
      const mapped: EwaTenantOutstandingRow[] = list.map((item: any) => ({
    ...
    onError: (error) => {
      console.error("Error loading EWA tenant outstanding data:", error)
      setRows([])
    },
  })
