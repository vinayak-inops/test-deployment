...
import { X, BadgeDollarSign, CalendarDays } from "lucide-react"
...
import {
  defaultEwaTenantOutstandingValues,
  ewaTenantOutstandingSchema,
  type EwaTenantOutstandingFormValues,
} from "./schemas/ewa-tenant-outstanding-form.schema"
...
export default function EwaTenantOutstandingAddPopup({
  isOpen,
  mode = "add",
  recordId = null,
  onClose,
  ...
  const [formData, setFormData]         = useState<EwaTenantOutstandingFormValues>(
    defaultEwaTenantOutstandingValues
  )
  ...
  const { loading: recordLoading, refetch: fetchRecord } = useRequest<any[]>({
    url:          "EWA_tenant_outstanding/search",
    method:       "POST",
  ...
  useEffect(() => {
    if (isOpen) {
      if ((isEditMode || isViewMode) && recordId && searchApiCriteria) {
        ...
        setFormData(defaultEwaTenantOutstandingValues)
  ...
  const { post } = usePostRequest<any>({
    url: "EWA_tenant_outstanding",
    onSuccess: async () => {
      toast.success(
        `EWA tenant outstanding record ${isEditMode ? "updated" : "created"} successfully`
      )
      await onSaved?.()
      setLoading(false)
      onClose()
  ...
  const submit = async () => {
    ...
    setShowErrors(true)
    const parsed = ewaTenantOutstandingSchema.safeParse(formData)
    if (!parsed.success) {
    ...
    try {
      await post({
        ...
        collectionName: "EWA_tenant_outstanding",
        event:          "ewaTenantOutstanding",
        data: {
          ...parsed.data,
          tenantCode,
  ...
  const setField = <K extends keyof EwaTenantOutstandingFormValues>(
    key: K,
    value: EwaTenantOutstandingFormValues[K]
  ) => {
    if (isViewMode) return
          ...
          <CardHeader className="px-6 py-3 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base font-semibold text-gray-700 flex items-center gap-2">
                ...
                EWA Tenant Outstanding — {getTitle()}
              </CardTitle>
              <button
                type="button"
                onClick={onClose}
              ...
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-3 h-9">
                        <button
                          ...
                          onClick={() => {
                            if (isViewMode) return
                            const next = !formData.paid
                            setField("paid", next)
                            if (!next) setField("paidOn", null)
                          }}
                    ...
                    {/* Paid On — visible only when paid = true */}
                    {formData.paid && (
                      <div className="space-y-2">
                        <Label className="block text-xs font-medium text-gray-700 uppercase tracking-wide flex items-center gap-1">
