...
import { Edit, MoreVertical, Loader2, WalletCards, ChevronLeft, ChevronRight } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Table, TableBody, TableCell, TableHead, TableHeader as DataTableHeader, TableRow } from "@/components/ui/table"
import type { EWAAllowedWithdrawalRow } from "./hooks/useewaallowedwithdrawallogic"

// ─── Types ────────────────────────────────────────────────────────────────────

interface EWAAllowedWithdrawalTableProps {
  rows: EWAAllowedWithdrawalRow[]
  loading: boolean
...
function PaginationControls({
  ...
  EWAAllowedWithdrawalTableProps,
...
function CurrencyCell({ value }: { value: number | string }) {
...
function AvailableCell({ available, limit }: { available: number | string; limit: number | string }) {
  if (available === "-" || limit === "-") return <span className="text-sm text-gray-400">-</span>

  const avail = Number(available)
  const lim = Number(limit)
  const percentage = lim > 0 ? (avail / lim) * 100 : 0
...
export default function EWAAllowedWithdrawalTable({
  rows,
  loading,
  countLoading,
  totalCount,
  ...
  if (loading || countLoading) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">Loading EWA allowed withdrawal records...</p>
      </div>
    )
  }
  ...
  if (rows.length === 0) {
    return (
      <div className="py-16 text-center">
        <WalletCards className="mx-auto mb-2 h-8 w-8 text-muted-foreground" />
        <p className="text-sm text-muted-foreground">No EWA allowed withdrawal records found</p>
      </div>
    )
  }
        ...
        <Table className="min-w-[800px]">
          <DataTableHeader>
            <TableRow className="bg-gray-50/80 border-b border-gray-200 hover:bg-gray-50/80">
              ...
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Limit</TableHead>
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Withdrawn</TableHead>
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Available</TableHead>
            ...
            {rows.map((row, index) => (
              ...
              >
                ...
                <TableCell className="px-5 py-2">
                  <CurrencyCell value={row.limit} />
                </TableCell>

                <TableCell className="px-5 py-2">
                  <CurrencyCell value={row.withdrawn} />
                </TableCell>

                <TableCell className="px-5 py-2">
                  <AvailableCell available={row.available} limit={row.limit} />
                </TableCell>
