...
import { Edit, MoreVertical, Loader2, UserCog, ChevronLeft, ChevronRight } from "lucide-react"
...
import type { EWAEmployeeSettingsRow } from "./hooks/Useewaemployeesettingslogic"

// ─── Types ────────────────────────────────────────────────────────────────────

interface EWAEmployeeSettingsTableProps {
  rows: EWAEmployeeSettingsRow[]
  loading: boolean
...
function AllowedPercentageCell({ value }: { value: number | string }) {
...
export default function EWAEmployeeSettingsTable({
  rows,
  loading,
  countLoading,
  totalCount,
  ...
  if (loading || countLoading) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">Loading EWA employee settings...</p>
      </div>
    )
  }
  ...
  if (rows.length === 0) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">No EWA employee settings found</p>
      </div>
    )
  }
        ...
        <Table className="min-w-[800px]">
          <DataTableHeader>
            <TableRow className="bg-gray-50/80 border-b border-gray-200 hover:bg-gray-50/80">
              ...
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Allowed Percentage</TableHead>
              <TableHead className="px-5 py-2.5 whitespace-nowrap text-xs font-medium text-gray-600">Blacklist</TableHead>
          ...
          <TableBody>
            {rows.map((row, index) => (
              ...
              >
                ...
                <TableCell className="px-5 py-2 text-sm">{row.categoryCode}</TableCell>

                <TableCell className="px-5 py-2">
                  <AllowedPercentageCell value={row.allowedPercentage} />
                </TableCell>

                <TableCell className="px-5 py-2">
                  <BlacklistBadge isBlacklisted={row.isBlacklisted} />
                </TableCell>
