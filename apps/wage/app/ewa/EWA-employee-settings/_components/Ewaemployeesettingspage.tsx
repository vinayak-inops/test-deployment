...
import {
  useEWAEmployeeSettingsLogic,
  FIELD_LABELS,
  type EWAEmployeeSettingsSearchField,
} from "./hooks/Useewaemployeesettingslogic"
import EWAEmployeeSettingsTable from "./Ewaemployeesettingstable"
import AddEWAEmployeeSettingModal, { type EWAEmployeeSettingItem } from "./AddEWAEmployeeSettingModal"
...
export default function EWAEmployeeSettingsPage() {
  ...
  const [editData, setEditData] = useState<EWAEmployeeSettingItem | null>(null)
  const shouldLoadEditData = (isEditOpen || isViewOpen) && !!rowId

  useRequest<any[]>({
    url: "EWA_employee_settings/search",
    method: "POST",
  ...
  return (
    <div>
      {/* Header */}
      <TableHeader
        title="EWA Employee Settings"
        description={
          loading || countLoading
            ? "EWA Employee Settings Management | Loading..."
            : `EWA Employee Settings Management | ${totalCount} records`
        }
        canAdd={addMode}
        onAddNew={() => router.push("/ewa/EWA-employee-settings?mode=add")}
        ...
        <div className="max-w-7xl mx-auto flex items-center gap-4">
          <div className="flex bg-muted/50 rounded-lg border flex-1">
            ...
            <div className="flex items-center bg-background border-r rounded-l-lg px-3 py-2 w-52">
              <Select
                value={searchField}
                onValueChange={(v) => setSearchField(v as EWAEmployeeSettingsSearchField)}
              >
