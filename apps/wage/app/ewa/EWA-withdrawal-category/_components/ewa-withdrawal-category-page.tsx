...
import {
  useEWAWithdrawalCategoryLogic,
  FIELD_LABELS,
  type EWAWithdrawalCategorySearchField,
} from "./hooks/Useewawithdrawalcategorylogic"
import EWAWithdrawalCategoryTable from "./ewa-withdrawal-categorytable"
import AddEWAWithdrawalCategoryModal, { type EWAWithdrawalCategoryItem } from "./AddEWAWithdrawalCategoryModal"
...
export default function EWAWithdrawalCategoryPage() {
  ...
  const [editData, setEditData] = useState<EWAWithdrawalCategoryItem | null>(null)
  ...
  useRequest<any[]>({
    ...
    onSuccess: (data) => {
      ...
      setEditData({
        ...
        allowedPercentage: Number(item?.allowedPercentage ?? 0),
        maxWithdrawalPerRequest: Number(item?.maxWithdrawalPerRequest ?? 0),
        minWithdrawalPerRequest: Number(item?.minWithdrawalPerRequest ?? 0),
        maxWithdrawalsPerMonth: Number(item?.maxWithdrawalsPerMonth ?? 0),
        coolingPeriodHours: Number(item?.coolingPeriodHours ?? 0),
        includeComponents: Array.isArray(item?.includeComponents) ? item.includeComponents : [],
  ...
  return (
    <div>
      {/* Header */}
      <TableHeader
        title="EWA Withdrawal Categories"
        description={
          loading || countLoading
            ? "EWA Withdrawal Category Management | Loading..."
            : `EWA Withdrawal Category Management | ${totalCount} categories`
        }
        canAdd={addMode}
        onAddNew={() => router.push("/ewa/EWA-withdrawal-category?mode=add")}
        ...
        <div className="max-w-7xl mx-auto flex items-center gap-4">
          <div className="flex bg-muted/50 rounded-lg border flex-1">
            ...
            <div className="flex items-center bg-background border-r rounded-l-lg px-3 py-2 w-52">
              <Select
                value={searchField}
                onValueChange={(v) => setSearchField(v as EWAWithdrawalCategorySearchField)}
              >
                <SelectTrigger className="w-full h-6 border-none p-0 text-sm font-medium text-foreground focus:ring-0 bg-transparent shadow-none">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="categoryTitle">Category Title</SelectItem>
                  <SelectItem value="categoryCode">Category Code</SelectItem>
                </SelectContent>
              </Select>
            </div>
