...
import { Trash2, Filter, Search as SearchIcon, Check, CalendarCheck, X } from "lucide-react"
...
function normalizeMinimumWagesValues(value: any): MinimumWagesPopupValues {
  return {
    country: safeStr(value?.country, "INDIA"),
    state: safeStr(value?.state),
    zone: safeStr(value?.zone),
    location: safeArray(value?.location),
    skillLevel: {
      skilledLevelTitle: safeStr(value?.skillLevel?.skilledLevelTitle),
      skilledLevelDescription: safeStr(value?.skillLevel?.skilledLevelDescription),
    },
    effectiveFrom: safeStr(value?.effectiveFrom),
    salaryHeads: safeArray(value?.salaryHeads),
    remark: safeStr(value?.remark),
  }
}

export default function MinimumWagesForm({
  mode,
  rowId,
  ...
  const isCountryStateSkillDisabled = isViewMode || isEditMode
  ...
  const { loading: recordLoading, refetch: fetchRecord } = useRequest<any[]>({
    ...
    onSuccess: (data: any[]) => {
      ...
      if (record) {
        reset(normalizeMinimumWagesValues(record))
      } else if (rowId) {
    ...
    onError: () => {
      toast.error("Failed to load minimum wages data")
      setFetchingData(false)
      onClose()
    },
  })
  ...
  useEffect(() => {
    if ((isEditMode || isViewMode) && rowId && searchApiCriteria) {
      ...
      reset(defaultMinimumWagesPopupValues)
      setFetchingData(false)
    }
  ...
  const onSubmit = async ({ salaryHeads: _salaryHeads, ...data }: MinimumWagesPopupValues) => {
    setLoading(true)
    await post({
      tenant: tenantCode,
  ...
  const getTitle = () => {
    if (isViewMode) return "View Minimum Wages"
    if (isEditMode) return "Edit Minimum Wages"
    return "Add Minimum Wages"
  }
  ...
  return (
    <div className="w-full space-y-6">
      <Card className="w-full mx-auto border border-gray-200 bg-white shadow-sm">
        <GradientFormHeader
          icon={CalendarCheck}
          title={`Minimum Wages - ${getTitle()}`}
          description="Define country, state, skill level, zone, effective date, and location mapping for minimum wages."
        />

        <CardContent className="px-6 py-4 space-y-6">
          {isLoading ? (
            <div className="py-12 text-center text-sm text-gray-600">Loading minimum wages data...</div>
          ) : (
            <form onSubmit={handleSubmit(onSubmit, handleInvalidSubmit)} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-2">
                        <SingleSelectField
                          ...
                          disabled={isCountryStateSkillDisabled}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          disabled={isViewMode}
                        >
                          <SelectTrigger className={`${fieldStyles} ${disabledFieldStyles} ${errors.zone ? "border-red-500" : ""}`}>
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          className={`${fieldStyles} ${disabledFieldStyles} ${errors.effectiveFrom ? "border-red-500" : ""}`}
                        />
              ...
              {activeTab === "salaryHeads" && (
                <div className="text-sm text-gray-500">Salary Heads configuration will be displayed here.</div>
              )}
