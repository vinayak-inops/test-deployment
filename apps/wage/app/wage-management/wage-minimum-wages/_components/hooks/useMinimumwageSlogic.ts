...
export type MinimumWagesSearchField =
  | "country"
  | "state"
  | "applicableTo"
  | "effectiveFrom"
...
export const FIELD_LABELS: Record<MinimumWagesSearchField, string> = {
  country: "Country",
  state: "State",
...
export function useMinimumWagesLogic() {
  ...
  const [rows, setRows] = useState<MinimumWagesRow[]>([])
  const [searchField, setSearchField] = useState<MinimumWagesSearchField>("state")
  ...
  const { loading: countLoading, refetch: refetchCounter } = useRequest<any>({
    ...
    onError: (e) => {
      console.error("Failed to load minimum wages count", e)
      setTotalCount(0)
    },
  })
  ...
  const { loading, refetch } = useRequest<any[]>({
    url: `wageMinimumWages/search?offset=${offset}&limit=${PAGE_SIZE}`,
    method: "POST",
    data: apiCriteria,
    ...
    onSuccess: (data) => {
      ...
      const mapped: MinimumWagesRow[] = list.map((item: any) => ({
        ...
        salaryHeads: Array.isArray(item?.salaryHeads)
          ? item.salaryHeads.map((sh: any) => ({
              salaryHeadCode: sh?.salaryHeadCode || "",
              salaryHeadName: sh?.salaryHeadName || "",
              amount: sh?.amount ?? 0,
              parseID: sh?.parseID || "",
            }))
          : [],
        ...
        slabs: Array.isArray(item?.slabs)
          ? item.slabs.map((slab: any) => ({
              from: slab?.from ?? 0,
              to: slab?.to ?? 0,
              amount: slab?.amount ?? 0,
              parseID: slab?.parseID || "",
            }))
          : [],
    ...
    onError: (error) => {
      console.error("Error loading minimum wages data:", error)
      setRows([])
    },
  })
