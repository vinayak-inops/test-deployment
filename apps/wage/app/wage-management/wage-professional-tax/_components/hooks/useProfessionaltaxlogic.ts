...
export type ProfessionalTaxSearchField =
  | "country"
  | "state"
  | "applicableTo"
  | "effectiveFrom"
...
export type ProfessionalTaxRow = {
  _id: string
  country: string
  state: string
  applicableTo: string
...
export const FIELD_LABELS: Record<ProfessionalTaxSearchField, string> = {
  country: "Country",
  state: "State",
...
export function useProfessionalTaxLogic() {
  ...
  const [rows, setRows] = useState<ProfessionalTaxRow[]>([])
  const [searchField, setSearchField] = useState<ProfessionalTaxSearchField>("state")
  ...
  const { loading: countLoading, refetch: refetchCounter } = useRequest<any>({
    ...
    onError: (e) => {
      console.error("Failed to load professional tax count", e)
      setTotalCount(0)
    },
  })
  ...
  const { loading, refetch } = useRequest<any[]>({
    url: `wageProfessionalTax/search?offset=${offset}&limit=${PAGE_SIZE}`,
    method: "POST",
    data: apiCriteria,
    ...
    onSuccess: (data) => {
      ...
      const mapped: ProfessionalTaxRow[] = list.map((item: any) => ({
    ...
    onError: (error) => {
      console.error("Error loading professional tax data:", error)
      setRows([])
    },
  })
