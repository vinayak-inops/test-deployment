...
export default function EmployerLWFContributionsFormPopup({
  open,
  isViewMode,
  initialValue,
  mode = "add",
  ...
  const {
    ...
    defaultValues: createEmptyLWFContribution("employer"),
  })
  ...
  const { post, loading } = usePostRequest<any>({
    ...
    onSuccess: async (response) => {
      ...
      toast.success(`Employer LWF contribution ${initialValue ? "updated" : "saved"} successfully`)
    ...
    onError: () => {
      pendingRowsRef.current = null
      toast.error("Failed to save employer LWF contribution")
    },
  })
  ...
  useEffect(() => {
    ...
    if (initialValue) {
      ...
      reset(createEmptyLWFContribution("employer"))
    }
  ...
  const handleFormSubmit = (data: LWFContributionItem) => {
    ...
    // Build the full updated employer rows — EDIT replaces at index, ADD appends
    const existingRows = (lwf.employer || []).map((row, index) => ({
      ...row,
      contributionType: "employer" as const,
      parseID: `employer-${row.state || "state"}-${row.effectiveFrom || "date"}-${index}`,
    }))
    ...
    const serverEmployerRows = nextRows.map((row, idx) => {
    ...
    post?.({
      ...
      event: "validate",
      ruleId: "wageEmployerContributionsLWFEmployerValidator",
      data: {
        lwf: {
          employer: serverEmployerRows,
        },
        tenantCode,
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="w-full h-full flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-900">
                  {initialValue ? "Edit Employer LWF Contribution" : "Add Employer LWF Contribution"}
                </h3>
          ...
          <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1 min-h-0">
            <div className="space-y-3">
              <SubFormTitle title="Employer LWF Contribution Information" />
          ...
          <div className="px-5 py-3 border-t border-gray-200 bg-gray-50 rounded-b-lg flex justify-end gap-2 flex-shrink-0">
            ...
            >
              {loading ? "Saving..." : initialValue ? "Save" : "Add Employer LWF"}
            </Button>
          </div>
        </form>
      </div>
