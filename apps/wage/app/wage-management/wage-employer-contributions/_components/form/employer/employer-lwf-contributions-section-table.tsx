...
import EmployerLWFContributionsFormPopup from "./employer-lwf-contributions-form-popup"
import {
...
export default function EmployerLWFContributionsSectionTable({
  mode = "add",
  rowId = null,
  ...
  const { arrayData: fetchedEmployerLwf, loading, refetch } = useAggregateArrayFetch<any>({
    collection: "wageEmployerContributions",
    criteriaRequests,
    arrayField: "lwf.employer",
    enabled: canFetchLwf,
    defaultValue: [],
  })
  ...
  const { post: postDelete, loading: deleteLoading } = usePostRequest<any>({
    ...
    onSuccess: async () => {
      ...
      setDeleteIndex(null)
      toast.success("Employer LWF contribution removed successfully")
      await refetch()
      await onSaved?.()
    },
    onError: () => {
      pendingDeleteRowsRef.current = null
      toast.error("Failed to remove employer LWF contribution")
    },
  })
  ...
  useEffect(() => {
    if (!canFetchLwf) return
    if (!Array.isArray(fetchedEmployerLwf)) return
    setLwf((prev) => ({
      ...prev,
      employer: fetchedEmployerLwf.map((item: any) => ({
  ...
  }, [canFetchLwf, fetchedEmployerLwf])
  ...
  const updateRows = (nextRows: LWFContributionItem[]) =>
    setLwf((prev) => ({
      ...prev,
      employer: nextRows.map(({ contribution, country, state, effectiveFrom }) => ({
        contribution,
        country,
        state,
        effectiveFrom,
  ...
  const persistDeleteRows = (nextRows: LWFContributionItem[]) => {
    const nextEmployerRows = nextRows.map(({ contribution, country, state, effectiveFrom }) => ({
      contribution,
      country,
      state,
      effectiveFrom,
  ...
  return (
    <div className="relative bg-white border border-gray-200 rounded-lg shadow-sm">
      <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-2.5">
        ...
        <div>
          <h2 className="text-[13px] font-semibold text-gray-900 leading-none">
            Employer LWF Contributions ({rows.length})
          </h2>
          <p className="text-[11px] text-gray-500 mt-0.5 leading-snug">
            Add or edit employer Labour Welfare Fund contribution configurations.
          </p>
        </div>
      </div>
      ...
      <div className="px-6 py-4 space-y-4">
        {loading ? (
          <div className="py-8 text-center text-sm text-gray-600">Loading employer LWF contributions...</div>
        ) : (
          <ActionDataTable<LWFContributionItem>
            ...
            addButtonLabel="Add Employer LWF"
            ...
            emptyTitle="No employer LWF contributions added yet."
            emptyDescription="Use Add Employer LWF to create entries."
          />
        )}
      </div>

      <EmployerLWFContributionsFormPopup
        open={open}
        isViewMode={isViewMode}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white border border-red-300 rounded-lg shadow-xl max-w-md w-full mx-4">
            <div className="px-5 py-4 border-b border-red-100 flex items-center gap-3 bg-red-50 rounded-t-lg">
              ...
              <div>
                <h3 className="text-sm font-semibold text-red-900">Remove employer LWF contribution</h3>
                <p className="text-[11px] text-red-600 mt-0.5">
                  Are you sure you want to remove this employer LWF contribution?
                </p>
              </div>
            </div>
