...
import type { WageSalaryHeadRow, SalaryHeadEntry } from "./hooks/useWagesalaryHeadsLogic"
import { useState } from "react"

// ─── Types ────────────────────────────────────────────────────────────────────

interface WageSalaryHeadsTableProps {
  rows: WageSalaryHeadRow[]
  loading: boolean
...
function SalaryHeadsSummary({ heads, label }: { heads: SalaryHeadEntry[]; label: string }) {
  const [isHovered, setIsHovered] = useState(false)
...
function NestedInfo({ primary, secondary }: { primary: string; secondary?: string }) {
  return (
    <div className="flex flex-col gap-0.5">
      ...
      {secondary && secondary !== "-" && (
        <span className="text-xs text-gray-400 truncate">{secondary}</span>
      )}
    </div>
  )
}
...
}: Pick<WageSalaryHeadsTableProps, "totalCount" | "totalPages" | "safePage" | "startIndex" | "endIndex" | "setCurrentPage">) {
  return (
...
export default function WageSalaryHeadsTable({
...
}: WageSalaryHeadsTableProps) {
  if (loading || countLoading) {
    return (
      <div className="py-16 text-center">
        <Loader2 className="mx-auto mb-2 h-8 w-8 text-blue-600 animate-spin" />
        <p className="text-sm text-muted-foreground">Loading salary heads...</p>
      </div>
    )
  }
  ...
  if (rows.length === 0) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">No salary heads found</p>
      </div>
    )
  }
  ...
  const col = {
    slNo:          { min: 70,  max: 70  },
    name:          { min: 140, max: 170 },
    code:          { min: 95,  max: 110 },
    description:   { min: 150, max: 180 },
    salaryType:    { min: 95,  max: 110 },
    primaryHead:   { min: 140, max: 170 },
    calculation:   { min: 95,  max: 110 },
    printable:     { min: 80,  max: 95  },
    basis:         { min: 140, max: 170 },
    excludedDays:  { min: 105, max: 120 },
    months:        { min: 115, max: 140 },
          ...
          <DataTableHeader>
            <TableRow className="bg-gray-50/80 border-b border-gray-200 hover:bg-gray-50/80">
              {[
                ...
                ["Salary Type",    col.salaryType],
                ["Primary Head",   col.primaryHead],
                // ["Calculation",    col.calculation],
                // ["Printable",      col.printable],
                ["Calculation Basis", col.basis],
                ["Excluded Days",  col.excludedDays],
                ["Applicable Months", col.months],
          ...
          <TableBody>
            {rows.map((row, index) => (
              ...
              >
                ...
                {/* Description */}
                <TableCell className="px-5 py-2 text-sm">
                  <TruncatedText text={String(row.raw?.description || "-")} maxWidth={col.description.max} />
                </TableCell>

                {/* Salary Type */}
                <TableCell className="px-5 py-2 text-sm">
                  <Badge variant="outline" className="text-xs capitalize">
                    {String(row.raw?.salaryType || "-")}
                  </Badge>
                </TableCell>
                ...
                {/* <TableCell className="px-5 py-2 text-sm">
                  <NestedInfo
                    primary={String(row.raw?.primarySalaryHead?.name || "-")}
                    secondary={String(row.raw?.primarySalaryHead?.code || "-")}
                  />
                </TableCell> */}
                ...
                {/* <TableCell className="px-5 py-2 text-sm">
                  <Badge variant="outline" className="text-xs capitalize">
                    {String(row.raw?.calculationType?.type || "-")}
                  </Badge>
                </TableCell> */}
                ...
                <TableCell className="px-5 py-2 text-sm">
                  <TruncatedText text={String(row.raw?.salaryCalculationBasis || "-")} maxWidth={col.basis.max} />
                </TableCell>

                {/* Excluded Days */}
                <TableCell className="px-5 py-2 text-sm">
                  <Badge variant="outline" className="text-xs">
                    {Array.isArray(row.raw?.excludedDays) ? row.raw.excludedDays.length : 0} days
                  </Badge>
                </TableCell>
                ...
                <TableCell className="px-5 py-2 text-sm">
                  <Badge variant="outline" className="text-xs">
                    {Array.isArray(row.raw?.applicableMonths) ? row.raw.applicableMonths.length : 0} months
                  </Badge>
                </TableCell>
