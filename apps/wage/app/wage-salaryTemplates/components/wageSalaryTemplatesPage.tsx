...
import WageSalaryTemplates from "./wageSalaryTemplates"
...
export default function WageSalaryTemplatesPage() {
  ...
  const { responseData: rolePermissions } = useRolePermissions({
    serviceName: "wage", // adjust if needed
  ...
  const isSalaryTemplateForm = isFormMode && formType === "salary-template"
  ...
  const {
    ...
    url: `map/wageSalaryTemplates/search?tenantCode=${tenantCode}`,
    onSuccess: (data: any) => {

      // Handle the case where data is an array or single record
      let salaryTemplateItems: any[] = []

      if (Array.isArray(data)) {
        // If data is an array, use it directly
        salaryTemplateItems = data
      } else if (data && data.salaryTemplates && Array.isArray(data.salaryTemplates)) {
        // If data is a single record with salaryTemplates array, extract the array
        salaryTemplateItems = data.salaryTemplates
      } else if (data && data.salaryTemplate) {
        // If data is a single record with salaryTemplate, wrap it in array
          salaryTemplateItems = [data]
      } else if (data) {
        // If data exists but no salaryTemplate, still wrap it
        salaryTemplateItems = [data]
      } else {
      }

      // Filter out deleted items
      const activeItems = salaryTemplateItems.filter((item: any) => {
        // Handle both nested and flat structures
        const checkItem = item.salaryTemplate || item
        return checkItem.isDeleted !== true && item.isDeleted !== true
      })

      const filteredData = activeItems.map((item: any, index: number) => {
        const st = item.salaryTemplate || item
        ...
        const effectiveFromValue = st.effectiveFrom || item.effectiveFrom

        return {
          _id: item._id || `template_${index}`,
          name: st.name || item.name || "",
          code: st.code || item.code || "",
          state: st.state || item.state || "",
          zone: st.zone || item.zone || "",
          effectiveFrom: parseDate(effectiveFromValue),
          skillLevel: st.skillLevel?.skilledLevelTitle || item.skillLevel?.skilledLevelTitle || "",
          independentHeadsCount: ((st.independentSalaryHeads || item.independentSalaryHeads) || []).length,
          dependentHeadsCount: ((st.dependentSalaryHeads || item.dependentSalaryHeads) || []).length,
          country: st.country || item.country || "",
        }
      })


      const dummyData = activeItems.map((item: any, index: number) => {
        const st = item.salaryTemplate || item
        return {
          _id: item._id || `template_${index}`,
          name: st.name || item.name || "",
    ...
    onError: (error: any) => {
      console.error("Error loading wage salary templates data:", error)
      toast.error("Failed to load wage salary templates data")
      setSubOrganization([])
    },
  })
  ...
  const {
    ...
    onSuccess: (data) => {
      toast.success("Wage Salary Template deleted successfully!")
    },
    onError: (error) => {
      toast.error("Wage Salary Template deletion failed!")
  ...
  const functionalityList = {
    ...
    buttons: {
      ...
      addnew: {
        ...
        function: () => router.push("/wage-salaryTemplates?mode=add"),
      },
      cancel: {
        label: "Cancel",
        status: false,
      ...
      actionLink: {
        ...
        function: (data: any) => router.push(`/wage-salaryTemplates?mode=view&id=${data._id}`),
      },
      actionEdit: {
        label: "Edit",
        status: editMode,
        ...
        function: (data: any) => router.push(`/wage-salaryTemplates?mode=edit&id=${data._id}`),
      },
    },
  }
  ...
  // If in form mode, show the WageSalaryTemplates form
  if (modeCheck === "add" || modeCheck === "edit" || modeCheck === "view") {
    return (
      <div className="px-12 py-4">
        ...
        <WageSalaryTemplates scrollToIndex={undefined} initialData={undefined} />
      </div>
    )
  }
  ...
  return (
    <div>
      <EnhancedHeader
        title={"Manage Wage Salary Templates"}
        description={"Manage Wage Salary Templates"}
      ...
      {!error &&
        (subOrganization && subOrganization.length > 0 ? (
          ...
          <div className="text-center py-8 text-gray-500">
            {loading ? "Loading..." : "No salary template data found"}
          </div>
        ))}
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="mb-4">
              <p className="text-gray-700">
                Are you sure you want to delete this wage salary template? This will permanently remove the record from
                the system.
              </p>
              {deleteLoading && (
