...
import { management, navItems, navItemsEwa } from "@/json/menu/menu";
...
function DashboardWrapper({
  ...
  const wageNav: any = useFilteredNavigationByObjectKey(navItems, roleData, ["wage"])
  const ewaNav: any = useFilteredNavigationByObjectKey(navItemsEwa, roleData, ["ewa"])
 
  const managementNav = useMemo(() => {
    const canAccess = {
      wageNav: wageNav.length > 0,
      ewaNav: ewaNav.length > 0
    };

    return management.filter((item: any) => {
      switch (item?.link) {
        case "/ewa":
          return canAccess.wageNav;
        case "/":
          return canAccess.ewaNav;
        default:
          return true;
      }
    });
...
useEffect(() => {
  ...
  if (pathname.startsWith("/ewa")) {
    setNavList(withDefault(ewaNav))
  } else {
    setNavList(withDefault(wageNav))
  }
}, [pathname, ewaNav, wageNav])
