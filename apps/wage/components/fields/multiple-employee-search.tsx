...
export default function EmployeeSearchField({
  ...
  const prevFiltersRef = useRef<{ designationCode?: string; gradeCodes?: string[]; employeeCategoryCodes?: string[] }>({})
  ...
  const queryVariables = useMemo(() => {
    ...
    if (employeeCategoryCodes && employeeCategoryCodes.length > 0) {
      criteriaRequests.push({
        field: 'deployment.employeeCategory.employeeCategoryCode',
        operator: 'in',
        value: employeeCategoryCodes,
      })
    }
    ...
    return vars
  }, [tenantCode, debouncedSearch, effectiveHierarchyFilters, userEntitlement, isOpen, organizationCode, preSelectedEmployeeId, limit, designationCode, gradeCodes, employeeCategoryCodes, isSelectAllActive])
  ...
  const hasRoleFilters = designationCode || (gradeCodes && gradeCodes.length > 0) || (employeeCategoryCodes && employeeCategoryCodes.length > 0)
  ...
  useEffect(() => {
    if (isSelectAllActive && multiSelect) {
      const currentFilters = { designationCode, gradeCodes, employeeCategoryCodes }
      ...
      prevFiltersRef.current = { designationCode, gradeCodes, employeeCategoryCodes }
    }
  }, [designationCode, gradeCodes, employeeCategoryCodes, isSelectAllActive, multiSelect, onMultiSelect, onSelectAllChange])
