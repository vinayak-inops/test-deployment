...
function mapBackendToPunchRequest(item: any): PunchRequest {
  return {
    ...
    status:
      item.workflowState?.toLowerCase().includes("approved")
        ? "approved"
...
export default function LeaveApplicationRequestsPopup({
 ...
 const canApprove =  (!!rolePermissions?.approve && modeOfRequest === 'applicationApplier') || (!!roleApprover?.approve && modeOfRequest === 'applicationApprover');
  ...
  const collectionName = useMemo(() => {
    ...
    if (userMode === 'approver') {
      if (workflowState?.includes('APPROVED') || workflowState?.includes('REJECTED') || workflowState?.includes('CANCELLED')) {
        return 'leaveApplicationTransaction'; // Using same collection for now, can be changed to Transaction if needed
      }
      // Pending and Failed use regular collection
      return 'leaveApplication';
    }

    // For user mode: always use regular collection (matching leave-application.tsx logic)
    // All, Pending, and all statuses use leaveApplication
    return 'leaveApplication';
  ...
  const getStatusColor = (status: string | undefined) => {
    ...
    if (statusLower.includes("pending") || statusLower.includes("initiated") || statusLower.includes("validated")) {
  ...
  const visibleRequests = punchRequests.filter(req => !req.workflowState?.toLowerCase().includes('cancel'));
                      ...
                      return (
                        <div className="space-y-2">
                          ...
                          {(() => {
                            ...
                            // If collection name is NOT 'leaveApplication' OR approver is false
                            // Check if workflow state matches processed states using == operator
                            ...
                            return false;
                          })() && (collectionName != 'leaveApplicationTransaction') && (
                ...
                {collectionName === 'leaveApplication' ? (
                  ...
                  <div className="flex items-center justify-center h-full px-6 py-4">
                    <div className="text-center">
                      <div className="mb-3">
                        {selectedRequest?.workflowState?.toUpperCase()?.includes('APPROVED') && (
                          <CheckCircle className="h-12 w-12 text-green-600 mx-auto" />
                        )}
                        {selectedRequest?.workflowState?.toUpperCase()?.includes('REJECTED') && (
                          <XCircle className="h-12 w-12 text-red-600 mx-auto" />
                        )}
                        {selectedRequest?.workflowState?.toUpperCase()?.includes('CANCELLED') && (
