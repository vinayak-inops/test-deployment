...
interface CompoffApplicationApproverProps {
    isApprovalPermission?: boolean
}

export default function CompoffApplicationApprover({ isApprovalPermission: _isApprovalPermission = false }: CompoffApplicationApproverProps) {
    ...
    const loggedInEmployeeId = employeeId || storedRoleInfo?.employeeId || storedRoleInfo?.employeeID || ""
    ...
    const buildRequestData = useMemo(() => {
        ...
        if (activeTab === 'pending' && loggedInEmployeeId) {
            requestData.push({
                field: "approverID",
        ...
        if (activeTab === 'failed' && loggedInEmployeeId) {
            requestData.push({
                field: "workflowState",
                value: "FAILED",
                operator: "like",
        ...
        if (activeTab === 'approved' && loggedInEmployeeId) {
            requestData.push({
                field: "approvedBy",
                value: loggedInEmployeeId,
                operator: "eq",
        ...
        if (activeTab === 'rejected' && loggedInEmployeeId) {
            requestData.push({
                field: "rejectedBy",
                value: loggedInEmployeeId,
                operator: "eq",
        ...
        if (activeTab === 'cancelled' && loggedInEmployeeId) {
            requestData.push({
                field: "cancelledBy",
                value: loggedInEmployeeId,
                operator: "eq",
    ...
    return (
        <div>
            <div className='px-0 py-3'>
                ...
                <p className='text-xs text-gray-500 mt-0.5'>Review and manage comp off applications assigned to you for approval</p>
            </div>
            <ApplicationFilters
                activeTab={activeTab}
