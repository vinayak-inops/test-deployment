...
export default function ApplicationTable({ data, onOpenDetails, loading = false, externalPagination }: ApplicationTableProps) {
  ...
  const currentData = useMemo(() => {
    if (useExternal) {
      // External pagination: data is already paginated on server
      return data
    }
    // Internal pagination: slice the sorted data
    return sortedData.slice(startIndex, endIndex)
  }, [useExternal, data, sortedData, startIndex, endIndex])
  ...
  const toggleSort = (key: keyof Application) => {
    ...
    // Use handlePageChange instead of setCurrentPage
    handlePageChange(1)
  }
        ...
        <CardContent className="px-0 py-0">
          {loading ? (
              ...
              <Table className="min-w-[1200px]">
                <TableHeader>
                  <TableRow className="bg-blue-50 hover:bg-blue-50">
                    ...
                    <TableHead className="w-[110px] px-5 py-2 whitespace-nowrap text-sm">Leave Code</TableHead>
                    <TableHead className="w-[100px] px-5 py-2 whitespace-nowrap text-sm">Balance</TableHead>
            ...
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-sm text-muted-foreground">No encashment applications found</p>
            </div>
          ) : (
            <>
              <div className="rounded-md border my-3 mx-3 overflow-x-auto [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full">
                  ...
                  <TableHeader>
                    <TableRow className="bg-blue-50 hover:bg-blue-50">
                      ...
                      <TableHead className="w-[110px] px-5 py-2 whitespace-nowrap text-sm">
                        <div className="flex items-center justify-between gap-2">
                          <span className="inline-flex items-center gap-1">Leave Code</span>
                          <ColumnSortMenu
                            onSortAsc={() => { setSortKey('leaveCode'); setSortDir('asc'); handlePageChange(1) }}
                            onSortDesc={() => { setSortKey('leaveCode'); setSortDir('desc'); handlePageChange(1) }}
                            onAlphaAsc={() => { setSortKey('leaveCode'); setSortDir('asc'); handlePageChange(1) }}
                            onAlphaDesc={() => { setSortKey('leaveCode'); setSortDir('desc'); handlePageChange(1) }}
                          />
                        </div>
                      </TableHead>
                      <TableHead className="w-[100px] px-5 py-2 whitespace-nowrap text-sm">
                        <div className="flex items-center justify-between gap-2">
                          <span className="inline-flex items-center gap-1">Balance</span>
                          <ColumnSortMenu
                            onSortAsc={() => { setSortKey('balance'); setSortDir('asc'); handlePageChange(1) }}
                            onSortDesc={() => { setSortKey('balance'); setSortDir('desc'); handlePageChange(1) }}
                          />
                        </div>
                      </TableHead>
                  ...
                  <TableBody>
                    {currentData.map((row, index) => (
                      ...
                      >
                        ...
                        <TableCell className="px-5 py-2 whitespace-nowrap text-sm">{row.leaveCode || '-'}</TableCell>
                        <TableCell className="px-5 py-2 whitespace-nowrap text-sm">{row.balance ?? '-'}</TableCell>
