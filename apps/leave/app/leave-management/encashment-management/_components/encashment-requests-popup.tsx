...
interface PunchRequestsPopupProps {
  ...
  // Optional: mode of the request (e.g., 'applicationApplier', 'applicationApprover')
...
export default function EncashmentManagementRequestsPopup({ 
  isOpen, 
  onClose, 
  ...
  const collectionName = useMemo(() => {
    ...
    if (userMode === 'approver') {
      if (workflowState === 'APPROVED' || workflowState === 'REJECTED' || workflowState === 'CANCELLED') {
        return 'leaveEncashmentApplicationTransaction'; // Using same collection for now, can be changed to Transaction if needed
      }
      // Pending and Failed use regular collection
      return 'leaveEncashmentApplication';
    }
    
    // For user mode: always use regular collection (matching encashment-application.tsx logic)
    // All, Pending, and all statuses use leaveEncashmentApplication
    return 'leaveEncashmentApplication';
    ...
    >
      <div className="bg-white w-full max-w-5xl h-[85vh] flex flex-col rounded-lg shadow-2xl border border-gray-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200">
          <h2 className="text-base font-semibold text-gray-700">Encashment Application</h2>
              <button
                onClick={onClose}
                        ...
                        {(() => {
                          ...
                          return (
                            <div className="space-y-2">
                              ...
                              {/* Leave Code */}
                              {selectedRequest.leaveCode && (
                                <div className="flex items-center border-b border-gray-100 pb-2">
                                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider w-32 flex-shrink-0">Leave Code</label>
                                  <span className="text-sm text-gray-900 font-medium">{selectedRequest.leaveCode}</span>
                                </div>
                              )}

                              {/* Balance */}
                              {typeof selectedRequest.balance !== 'undefined' && (
                                <div className="flex items-center border-b border-gray-100 pb-2">
                                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider w-32 flex-shrink-0">Balance</label>
                                  <span className="text-sm text-gray-900 font-medium">{selectedRequest.balance}</span>
                                </div>
                              )}
                ...
                onClick={() => {
                  ...
                  setStatusLoading(true);
                    ...
                    const data: any = {
                      ...
                      workflowName: selectedRequest?.workflowName || "leaveEncashment Application",
                      ...
                      balance: selectedRequest?.balance,
