...
const FETCH_LEAVE_POLICY_QUERY = gql`
...
export function EncashmentManagementPage({ onBack }: EncashmentManagementPageProps) {
  ...
  // GraphQL: same pattern as reference query (FetchLeavePolicy + criteriaRequests + collection)
  const {
    data: leavePolicyResponse,
    loading: leaveCodesLoading,
    error: leaveCodesError
  } = useQuery(FETCH_LEAVE_POLICY_QUERY, {
    client,
    variables: {
      criteriaRequests: [
        {
  ...
  const transformApiData = (apiData: any[]): TransformedLeaveCodeData[] => {
    ...
    return apiData
      ...
      .flatMap((item: any) => {
        const policies = Array.isArray(item.leavePolicy) ? item.leavePolicy : [item.leavePolicy]
        return policies.map((leavePolicy: any) => {
  ...
  const handleNewEncashmentSubmit = async () => {
    ...
    const payload = {
      tenant: tenantCode, // TODO: Replace with actual tenant if available
      action: "insert",
      collectionName: "leaveEncashmentApplication",
      id: "",
      event: "application",
      data: {
        tenantCode: tenantCode, // TODO: Replace with actual tenant code
