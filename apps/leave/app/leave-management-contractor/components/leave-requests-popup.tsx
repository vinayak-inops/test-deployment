...
interface LeaveRequestsPopupProps {
  isOpen: boolean
  onClose: () => void
  leaveApplications: LeaveApplication[]
  initialSelectedApplication?: LeaveApplication | null
  showOnlySelected?: boolean
}

export default function LeaveRequestsPopup({ isOpen, onClose, leaveApplications, initialSelectedApplication, showOnlySelected = false }: LeaveRequestsPopupProps) {
  const [selectedApplication, setSelectedApplication] = useState<LeaveApplication | null>(initialSelectedApplication || null)
  ...
  useEffect(() => {
    ...
    // If showOnlySelected is true, automatically show status view
    if (showOnlySelected && initialSelectedApplication) {
      setShowStatusView(true)
    }
  }, [initialSelectedApplication, showOnlySelected])
    ...
    <BigPopupWrapper open={isOpen} setOpen={onClose} content={undefined}>
      ...
      <div className="w-full h-full">
        <div className="px-6 pt-6 pb-6 border-b border-gray-200 bg-white">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-gray-900">
              {showOnlySelected ? "Leave Application Status" : (showStatusView ? "Leave Application Status" : "Leave Applications")}
            </h2>
            {showStatusView && !showOnlySelected && (
              <button
        ...
        <div className="flex w-full" style={{height: 'calc(100% - 72px)'}}>
          {/* Left Side - Application List (hidden when showing status or when showOnlySelected is true) */}
          {!showStatusView && !showOnlySelected && (
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-1">
                              <h3 className="font-semibold text-gray-900 truncate">
                                Leave Application - {application.employeeID}
                              </h3>
                              <span
                            ...
                            <div className="flex items-center space-x-2 text-sm text-gray-600 mb-1">
                              ...
                              <span>{formatDate(application.fromDate)} - {formatDate(application.toDate)}</span>
                            </div>
                            <div className="flex items-center space-x-2 text-xs text-gray-500 mb-2">
                              <Clock className="h-3 w-3" />
                              <span>Applied {getTimeAgo(application.appliedDate)}</span>
                              <span>•</span>
                              <span>{getLeaveSummary(application.leaves)}</span>
                            </div>
                            <p className="text-xs text-gray-600 line-clamp-2">{application.remarks || 'No remarks'}</p>
                          </div>
                        </div>
                      </div>
                    ))}
          ...
          <div className={`${showOnlySelected ? 'w-full' : 'w-1/2'} flex flex-col ${showOnlySelected ? '' : 'border-r border-gray-200'}`}>
            <div className="flex-1 overflow-y-auto custom-scroll">
              {selectedApplication ? (
                <div className="w-full p-6">
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <div className="text-base font-bold text-gray-900">Leave Application</div>
                  ...
                  <div className="space-y-3 mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-gray-700 text-sm">From Date:</span>
                      <span className="text-gray-900 text-sm">{formatDate(selectedApplication.fromDate)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4 text-gray-400" />
                      <span className="font-semibold text-gray-700 text-sm">To Date:</span>
                      <span className="text-gray-900 text-sm">{formatDate(selectedApplication.toDate)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="mb-4">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-semibold text-gray-700 text-sm">Leave Details:</span>
                    </div>
                    <div className="text-gray-800 leading-relaxed text-xs bg-gray-50 rounded-lg p-3 mt-1">
                      {getLeaveSummary(selectedApplication.leaves)} ({selectedApplication.leaves?.length || 0} day(s))
                    </div>
                  </div>
                  {selectedApplication.remarks && (
                    <div className="mb-4">
                      <div className="flex items-center gap-2 mb-1">
                        <MessageSquare className="h-4 w-4 text-gray-400" />
                        <span className="font-semibold text-gray-700 text-sm">Remarks:</span>
                      </div>
                      <div className="text-gray-800 leading-relaxed text-xs bg-gray-50 rounded-lg p-3 mt-1">
                        {selectedApplication.remarks}
                      </div>
                    ...
                    {selectedApplication.workflowState === 'INITIATED' || selectedApplication.workflowState === 'PENDING' ? (
                      showCancel ? (
                        <div className="w-full">
                          <label htmlFor="cancel-comment" className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-1">
          ...
          {showStatusView && (
            <div className={`${showOnlySelected ? 'w-full' : 'w-1/2'} flex flex-col`}>
              <LeaveStatusUpdate 
