...
export interface RequestAbsenceModalProps {
  isOpen: boolean
  onClose: () => void
...
export function RequestAbsenceModal({ isOpen, onClose, selectedDates, selectedFullDates, currentDate, onDateDelete, selectedEmployeeId, preferredMultiDateMode }: RequestAbsenceModalProps) {
  ...
  const { leaveCodes, loading, error } = useLeaveCodes(selectedEmp)
  ...
  const isPageLoading = tokenLoading || loading || empLoading;

  if (isPageLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-[300px] overflow-y-auto">
  ...
  return (
    <>
      <Dialog open={isOpen && !showDetailedModal && !showLeaveOfAbsenceModal} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-2xl p-0 gap-0 bg-white rounded-lg shadow-xl" aria-describedby="request-absence-description">
          ...
          <div className="px-6 pb-6">
            ...
            <div className="mb-6 h-[120px] overflow-y-auto">
              <h3 className="text-sm font-semibold text-slate-900 mb-3">Request Dates</h3>
              <div className="space-y-1">
                {(selectedFullDates && selectedFullDates.length > 0
                  ? [...selectedFullDates].sort((a, b) => a.getTime() - b.getTime()).map((d, idx) => (
                    <div key={`${d.toISOString()}-${idx}`} className="text-xs text-slate-700">
                      {formatFullDate(d)}
                    </div>
                  ))
                  : selectedDates
              ...
              <div className="mt-2 text-xs text-slate-600 font-medium">
                Total days: <span className="text-blue-600 font-bold">{selectedFullDates && selectedFullDates.length > 0 ? selectedFullDates.length : selectedDates.length}</span>
              </div>
            </div>
                ...
                {filteredAbsenceTypes.slice(0, 2).map((type) => (
                  <div key={type.id}>
                    ...
                    {type.id === "Time Away" && showTimeAwayDropdown && (
                      <div className="mt-2 ml-4 border-l-2 border-blue-200 pl-4 h-[200px] overflow-y-auto">
      ...
      <DetailedRequestModal
        ...
        preferredMultiDateMode={preferredMultiDateMode}
      />
      ...
      <LeaveOfAbsenceModal
        ...
        selectedEmployeeId={selectedEmployeeId}
        selectedLeaveCode={selectedAbsenceLeaveType}
        selectedDates={selectedDates}
        selectedFullDates={selectedFullDates}
        currentDate={currentDate}
      />
