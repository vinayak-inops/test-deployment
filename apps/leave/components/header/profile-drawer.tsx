...
function formatLocationLine(city?: unknown, state?: unknown) {
  const parts = [formatValue(city), formatValue(state)].filter((value) => value !== "Not provided")
...
export default function ProfileDrawer({ open, onClose, userName }: ProfileDrawerProps) {
  ...
  const workInformationFields = [
    ...
    { label: "Employee Category Title", value: employeeProfile?.deployment?.employeeCategory?.employeeCategoryTitle },
    { label: "Grade Title", value: employeeProfile?.deployment?.grade?.gradeTitle },
    { label: "Designation Name", value: employeeProfile?.deployment?.designation?.designationName },
    { label: "Location Name", value: employeeProfile?.deployment?.location?.locationName },
    { label: "Skill Level Title", value: employeeProfile?.deployment?.skillLevel?.skillLevelTitle },
    { label: "Skill Level Description", value: employeeProfile?.deployment?.skillLevel?.skillLevelDescription },
  ]
  ...
  const handleViewMore = () => {
    ...
    router.push(
      `${NEXT_PUBLIC_NEXTAUTH_URL}/master/profile/company-employee?mode=view&id=${encryptedData}`
    )
  }
        ...
        <div className="sticky top-0 z-[100] bg-[#f3f4f8] px-4 py-4">
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold tracking-wide text-slate-700">PROFILE</h2>
              <button
                type="button"
                onClick={onClose}
            ...
            <p className="mt-1 text-xs text-slate-500">Quick access to employee details and contact information</p>
          </motion.div>
        </div>
            ...
            {profileError && !profileLoading ? (
              <Card className="border border-slate-200 bg-white shadow-sm">
                <CardContent className="py-8 text-center">
                  <p className="text-sm text-slate-500">Failed to load employee profile.</p>
                </CardContent>
              </Card>
            ) : null}
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                      <div>
                        ...
                        <p className="mt-1 font-medium text-slate-900">{formatValue(employeeProfile?.employeeID)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-500">Joining Date</p>
                        <p className="mt-1 font-medium text-slate-900">{formatDate(employeeProfile?.joiningDate || employeeProfile?.dateOfJoining)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-500">Gender</p>
                        <p className="mt-1 font-medium text-slate-900">{formatValue(employeeProfile?.gender)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-500">Date of Birth</p>
                        <p className="mt-1 font-medium text-slate-900">{formatDate(employeeProfile?.birthDate)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-500">Blood Group</p>
                        <p className="mt-1 font-medium text-slate-900">{formatValue(employeeProfile?.bloodGroup)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-500">Nationality</p>
                        <p className="mt-1 font-medium text-slate-900">{formatValue(employeeProfile?.nationality)}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="border border-slate-200 bg-white shadow-sm">
                  <CardContent className="p-4">
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      <h4 className="text-sm font-semibold text-slate-900">Work Information</h4>
                    </div>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                      {workInformationFields.map((field) => (
                        <div key={field.label}>
