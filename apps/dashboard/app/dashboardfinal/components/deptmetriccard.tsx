        ...
        <div className="w-full min-h-[250px] p-4 flex justify-center">
            <ChartContainer config={config}>
                <BarChart 
                    accessibilityLayer 
                    data={data}
                    width={Math.max(300, data.length * 100)} // Adjusted width for department display
...
interface DeptMetricCardProps {
    ...
    selectedDepartment: string; // New prop for department filtering
}

export const DeptMetricCard: React.FC<DeptMetricCardProps> = ({
    icon: Icon,
    title,
    value,
    change,
    ...
    // Centralized user entitlement for department metrics (same as other dashboard components)
    ...
    // Use useRequest hook for fetching attendance data with department filter
    const {
        ...
        onSuccess: (data) => {
            ...
            // Generate chart data showing department name instead of shift codes
            ...
            const newChartData = [{
                label: selectedDepartment, // Show department name instead of shift codes
                m: totalMale,
                f: totalFemale
            }];
        ...
        dependencies: [attendanceType, selectedDepartment, formattedToday, tenantCode, hierarchyFilters]
    });
    ...
    // Use useRequest hook for fetching employee data with department filter
    const {
        ...
        onSuccess: (data) => {
            ...
            if (!Array.isArray(data) || data.length === 0) {
                console.warn("No department employee data received or data is not an array");
                setEmployeeData([]);
                return;
            }
    ...
    // Determine modal title based on card type and department
    const getModalTitle = () => {
        if (cardType === 'employee-table') {
            return `Department ${selectedDepartment} - Employee List`;
        } else {
            const attendanceTypeLabel = attendanceType ? 
                attendanceType.charAt(0).toUpperCase() + attendanceType.slice(1).replace(/([A-Z])/g, ' $1') : 
                'Chart';
            return `Department ${selectedDepartment} - ${title} - ${attendanceTypeLabel} Data`;
        }
    };
                ...
                <div className="flex flex-col h-full">
                    {isLoadingEmployees ? (
                        <div className="flex items-center justify-center h-64">
                            <div className="text-lg text-gray-600">Loading department employee data...</div>
                        </div>
                    ) : (
                        <div className="flex flex-col h-full">
                            ...
                            <div className="pt-4 border-t border-gray-200 flex justify-between items-center mt-4 flex-shrink-0 bg-white">
                                <div className="text-sm text-gray-600">
                                    {employeeData.length === 0 ? `No employee data available for department ${selectedDepartment}` : `${employeeData.length} employee(s) found in department ${selectedDepartment}`}
                                </div>
                                <div className="flex space-x-4">
                                    <button
                                        onClick={() => generateExcel(employeeData, `Department ${selectedDepartment} Employee List`)}
                                    ...
                                    <button
                                        onClick={() => generatePDF(employeeData, `Department ${selectedDepartment} Employee List`)}
                                        disabled={employeeData.length === 0}
                ...
                <div className="overflow-auto max-h-[70vh]">
                    <div className="min-w-[400px] flex justify-center">
                        <ChartModal 
                            data={chartData} 
                            config={chartConfig}
                            title={`Department ${selectedDepartment} - ${title}`}
                        />
                    </div>
                </div>
            );
        }
    }, [cardType, chartData, title, employeeData, isLoadingEmployees, tableFunctionalityList, chartConfig, selectedDepartment]);
            ...
            <div className="space-y-2">
                ...
                <div className="flex items-center text-sm">
                    <span className="text-white/80">
                        Among {formatValue(change)} Employees in Department {selectedDepartment}
                    </span>
                </div>
            </div>
        </div>
