        ...
        <div className="w-full min-h-[250px] p-4 flex justify-center">
            <ChartContainer config={config}>
                <BarChart 
                    accessibilityLayer 
                    data={data}
                    width={Math.max(300, data.length * 100)} // Adjusted width for work order display
...
interface WorkOrderMetricCardProps {
    ...
    selectedWorkOrder: string; // New prop for work order filtering
}

export const WorkOrderMetricCard: React.FC<WorkOrderMetricCardProps> = ({
    icon: Icon,
    title,
    value,
    change,
    ...
    // Use useRequest hook for fetching attendance data with work order filter
    const {
        ...
        onSuccess: (data) => {
            ...
            // Generate chart data showing work order name instead of shift codes
            ...
            const newChartData = [{
                label: selectedWorkOrder, // Show work order name instead of shift codes
                m: totalMale,
                f: totalFemale
            }];
        ...
        dependencies: [attendanceType, selectedWorkOrder, formattedToday, tenantCode, hierarchyFilters]
    });
    ...
    // Use useRequest hook for fetching employee data with work order filter
    const {
        ...
        onSuccess: (data) => {
            ...
            if (!Array.isArray(data) || data.length === 0) {
                console.warn("No work order employee data received or data is not an array");
                setEmployeeData([]);
                return;
            }
        ...
        onError: (error) => {
            console.error("Error fetching work order employee data:", error);
    ...
    // Determine modal title based on card type and work order
    const getModalTitle = () => {
        if (cardType === 'employee-table') {
            return `Work Order ${selectedWorkOrder} - Employee List`;
        } else {
            const attendanceTypeLabel = attendanceType ? 
                attendanceType.charAt(0).toUpperCase() + attendanceType.slice(1).replace(/([A-Z])/g, ' $1') : 
                'Chart';
            return `Work Order ${selectedWorkOrder} - ${title} - ${attendanceTypeLabel} Data`;
        }
    };
                ...
                <div className="flex flex-col h-full">
                    {isLoadingEmployees ? (
                        <div className="flex items-center justify-center h-64">
                            <div className="text-lg text-gray-600">Loading work order employee data...</div>
                        </div>
                    ) : (
                        <div className="flex flex-col h-full">
                            ...
                            <div className="pt-4 border-t border-gray-200 flex justify-between items-center mt-4 flex-shrink-0 bg-white">
                                <div className="text-sm text-gray-600">
                                    {employeeData.length === 0 ? `No employee data available for work order ${selectedWorkOrder}` : `${employeeData.length} employee(s) found in work order ${selectedWorkOrder}`}
                                </div>
                                <div className="flex space-x-4">
                                    <button
                                        onClick={() => generateExcel(employeeData, `Work Order ${selectedWorkOrder} Employee List`)}
                                    ...
                                    <button
                                        onClick={() => generatePDF(employeeData, `Work Order ${selectedWorkOrder} Employee List`)}
                                        disabled={employeeData.length === 0}
                ...
                <div className="overflow-auto max-h-[70vh]">
                    <div className="min-w-[400px] flex justify-center">
                        <ChartModal 
                            data={chartData} 
                            config={chartConfig}
                            title={`Work Order ${selectedWorkOrder} - ${title}`}
                        />
                    </div>
                </div>
            );
        }
    }, [cardType, chartData, title, employeeData, isLoadingEmployees, tableFunctionalityList, chartConfig, selectedWorkOrder]);
            ...
            <div className="space-y-2">
                ...
                <div className="flex items-center text-sm">
                    <span className="text-white/80">
                        Among {formatValue(change)} Employees in Work Order {selectedWorkOrder}
                    </span>
                </div>
            </div>
        </div>
