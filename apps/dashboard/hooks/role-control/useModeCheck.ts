...
import { useCurrentPageInfo, useExtractAllRoutes } from './useRoleControl';
        ...
        try {
            ...
            if (searchParams == "") {
                if (baseRouteExists) {
                    ...
                    // router.push(`${process.env.NEXT_PUBLIC_NEXTAUTH_URL}/launchdesk`);
                    setIsLoading(false);
                    return { exists: false, currentMode: null, newPageString: null };
                }
            }
            ...
            console.error('Error in handleModeCheck:', error);
            setIsLoading(false);
            return { exists: false, currentMode: null, newPageString: null };
        }
