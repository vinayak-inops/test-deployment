...
export function useUserEntitlement(
  ...
  return useMemo(() => {
    ...
    if (hierarchyFilters?.isManager !== undefined) {
      entitlement.isManager = hierarchyFilters.isManager
    }

    if (hierarchyFilters?.isEndUser !== undefined) {
      entitlement.isEndUser = hierarchyFilters.isEndUser
    }
