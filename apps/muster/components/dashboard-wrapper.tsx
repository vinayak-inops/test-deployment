...
import { menu } from "@/json/menu/menu";
...
function DashboardWrapper({
  ...
  const filteredNavItems = useFilteredNavigation(menu, adminRole);
  useEffect(() => {
    setNavList(filteredNavItems)
  }, [ filteredNavItems, menu]);
  

  return (
    <div className="bg-white h-screen overflow-y-auto scroll-hidden">
      ...
      <div className="flex flex-wrap h-full">
        ...
        >
          <div className="flex flex-col bg-[#f3f4f8] min-h-screen relative shadow-[0_48px_100px_0_rgba(17,12,46,0.15)]">
            {menu?.length > 0 && (
              // <MainHeader navItems={navList}/>
              <Header navItems={navList} serviceName="muster"/>
            )}
            <SideMenu />
