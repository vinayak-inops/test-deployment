...
import PunchFormPopup from "@/app/punch/_components/punch-form-popup"

type TabKey = "pending" | "approved" | "rejected" | "cancelled" | "applications" | "failed"
...
const TABS: { key: TabKey; label: string; activeBg: string; activeText: string; activeBorder: string; hoverBg: string; hoverText: string }[] = [
  { key: "applications", label: "Applications", activeBg: "bg-blue-100", activeText: "text-blue-800", activeBorder: "border-blue-400", hoverBg: "border-blue-400", hoverText: "hover:text-blue-700" },
...
export default function ApplicationFilters({ activeTab, onTabChange, onApply, isSelfPermission = false, isAllPermission = false, refreshAll, hideApplicationsTab = false, hideApplyButton = false }: ApplicationFiltersProps) {
  ...
  // For self: show applications, pending, approval, cancel, reject
  // For all: show applications, pending, approval, cancel, reject
  // For approver: hide applications tab
  const visibleTabs = useMemo(() => {
    let tabs = TABS
    if (hideApplicationsTab) {
      tabs = tabs.filter(tab => tab.key !== 'applications')
    }
