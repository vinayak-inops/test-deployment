...
interface PunchApplicationApproverProps {
    isApprovalPermission?: boolean
}

export default function PunchApplicationApprover({ isApprovalPermission = false }: PunchApplicationApproverProps) {
    const [otApplications, setOtApplications] = useState<any[]>([]);
    const [activeTab, setActiveTab] = useState<'pending' | 'approved' | 'rejected' | 'cancelled' | 'applications' | 'failed'>('pending');
    ...
    const collectionName = useMemo(() => {
        // Pending and Failed always use forgotPunchApplication
        if (activeTab === 'pending' || activeTab === 'failed') {
            return 'forgotPunchApplication'
        }
        // For Cancel, Reject, Approve: use forgotPunchApplicationTransaction
    ...
    const buildRequestData = useMemo(() => {
        ...
        // For approve tab: filter by workflowState and approvedBy
        ...
        // For reject tab: filter by workflowState and rejectedBy
        ...
        // For cancel tab: filter by workflowState and cancelledBy
