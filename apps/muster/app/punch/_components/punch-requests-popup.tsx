...
import { Clock, Send, CheckCircle, XCircle, LogIn, LogOut, Calendar, MessageSquare, User, Mail, Phone, AlertCircle, BarChart3, UserCheck, X, ChevronDown, Settings, ArrowLeft, RefreshCw } from "lucide-react"
...
// Get URL parameters (declare before using in hooks)
const getUrlParams = () => {
  if (typeof window !== 'undefined') {
    ...
    if (employeeId && month && year) {
      return { type: 'calendar', employeeId, month, year } as any;
    }
    if (employeeId && fromdate && todate) {
      return { type: 'table', employeeId, fromdate, todate } as any;
    }
  }
  return null;
};

export default function PunchRequestsPopup({ isOpen, onClose, initialSelectedRequest, selectedRequestId, employeeId }: PunchRequestsPopupProps) {
  ...
  const [musterService, setMusterService] = React.useState<any>(null);
   // Removed unused popup state
   const [excelService, setExcelService] = useState<any>(null);
   const [punchApprove, setPunchApprove] = useState<any>(null);
   const [punchReject, setPunchReject] = useState<any>(null);
   const [punchCancel, setPunchCancel] = useState<any>(null);
   const [punchKnowStatus, setPunchKnowStatus] = useState<any>(null);
   const [punchCancelRequest, setPunchCancelRequest] = useState<any>(null);
   const  tenantCode  = useGetTenantCode();
 
   const contractorEmployee = "punch-application"
   ...
   useEffect(() => {
     ...
     if (musterScreen) {
       const punchApprove = musterScreen.permissions[`punchApplicationsApprove`];
       const punchReject = musterScreen.permissions[`punchApplicationsReject`];
       const punchCancel = musterScreen.permissions[`punchApplicationsCancel`];
       const punchKnowStatus = musterScreen.permissions[`punchApplicationsKnowStatus`];
      const punchCancelRequest = musterScreen.permissions[`punchApplicationsCancelRequest`];
       setPunchApprove(punchApprove);
       setPunchReject(punchReject);
       setPunchCancel(punchCancel);
       setPunchKnowStatus(punchKnowStatus);
       setPunchCancelRequest(punchCancelRequest);
     }
   }, [contractorEmployee, excelService]);

  // Derive user role from Redux adminRole instead of cookies
  const userRole = React.useMemo(() => {
    const roleType = adminRole?.roleType || "";
    const employeeID = adminRole?.employeeId || adminRole?.employeeID || "";
    // Normalize to previous structure shape used in component
    return {
      role: roleType?.toLowerCase() === 'user' ? 'user' : roleType || 'admin',
      employeeID,
    } as any;
  }, [adminRole]);
  ...
  const postBody = React.useMemo(() => {
    // alert(employeeId)
    // 1) If specific request id is passed, filter by _id
    ...
    // 5) Default: no employee filter
    return [
      { field: "tenantCode", operator: "eq", value: tenantCode },
    ];
  }, [selectedRequestId, employeeId, userRole]);
  ...
  // (getUrlParams is declared above for fallback use in postBody)
  ...
  return (
    ...
    >
      <div className="bg-transparent w-full max-w-7xl h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={isLoading}
                  className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-md border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                  aria-label="Refresh"
                >
                  {isLoading ? (
            ...
            <div className="flex-1 overflow-y-auto custom-scroll">
              {selectedRequest ? (
                <div className="w-full p-6">
                    ...
                    <div className="relative manage-dropdown">
                      ...
                      {showManageDropdown && (
                        <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-200 py-2 z-10">
                          ...
                          {punchCancelRequest && !['APPROVED', 'REJECTED', 'CANCELLED'].includes(selectedRequest.workflowState?.toUpperCase() || '') && (
                            <button
                              onClick={() => {
                                setShowManageDropdown(false);
                  ...
                  <div className="space-y-6 mb-6">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-x-8 gap-y-2">
                        ...
                        <div className="font-mono text-base text-blue-800">
                          {(() => {
                            if (!selectedRequest.createdOn) return '-';
                            
                            const parseCreatedOn = (dateStr: string) => {
                              try {
                                let date: Date;
                            ...
                            return parseCreatedOn(selectedRequest.createdOn);
                          })()}
                        </div> */}
                  ...
                  {/* Action Buttons */}
                  {(punchApprove || punchReject || punchCancel) && !['APPROVED', 'REJECTED', 'CANCELLED'].includes(selectedRequest.workflowState?.toUpperCase() || '') && (
