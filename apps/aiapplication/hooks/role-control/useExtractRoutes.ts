 ...
 * @param serviceName - The service name to filter routes (default: 'main')
 * @returns Array of route strings
 */
export const useExtractRoutes = (adminRole: any, serviceName: string = 'main'): string[] => {
    return useMemo(() => {
...
export const useExtractAllRoutes = (adminRole: any): string[] => {
    return useMemo(() => {
        if (!adminRole?.screenPermissions) {
            // Silently return empty array - permissions are loading asynchronously
            return [];
        }
